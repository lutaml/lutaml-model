...
def summary_report(current_results, base_results, base, run_time, threshold)
  summary = { run_time: run_time, threshold: threshold, branch: current_branch, base: base, metrics: {}, regressions: [] }
...
def print_realtime_comparison(label, curr_metrics, base_metrics, threshold)
  ...
  puts sprintf("%<label>30s: %<base>s -> %<curr>s IPS (change: %<delta>s) [%<status>s]\n\n", label: label, base: base_str, curr: curr_str, delta: delta_str, status: status)
end
