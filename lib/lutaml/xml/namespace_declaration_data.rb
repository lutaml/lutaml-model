    ...
    class NamespaceDeclarationData
      ...
      def validate!
        unless %i[default prefix].include?(@format)
          raise ArgumentError,
                "Format must be :default or :prefix, got #{@format.inspect}"
        end

        unless %i[here inherited local_on_use].include?(@declared_at)
          raise ArgumentError,
                "declared_at must be :here, :inherited, or :local_on_use, got #{@declared_at.inspect}"
        end
        ...
        if @source && @source != :input
          raise ArgumentError,
                "Source must be nil or :input, got #{@source.inspect}"
        end

        # Validate prefix override if present
        if @prefix_override && !@prefix_override.is_a?(String)
          raise ArgumentError,
                "Prefix override must be a String, got #{@prefix_override.class}"
        end
