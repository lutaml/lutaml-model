    ...
    module Schema
      ...
      class XsdSchema
        ...
        def self.emit_xsd_attribute(xml, attr, rule, xsd_type, attr_type)
          ...
          return xs(xml, "attribute", attr_attrs) unless restricted || rule.documentation
        ...
        def self.emit_value_element(xml, attrs, xsd_type, attr_type, attr)
          facets = effective_facets(attr, attr_type)
          if facets.any?
            xs(xml, "element", attrs.except(:type)) do
              emit_restriction(xml, xsd_type, facets, attr_type)
            end
          else
            xs(xml, "element", attrs.merge(type: xsd_type))
          end
        end

        def self.emit_restriction(xml, base, facets, type)
          xs(xml, "simpleType") do
            xs(xml, "restriction", base: base) do
        ...
        def self.emit_facet(xml, element, value)
          ...
          xml.public_send(qn(element), value: value.to_s)
        end
