...
module Lutaml
  module Xml
      ...
      Class.new(Lutaml::Xml::Namespace) do
        self.uri uri if uri
        prefix_default prefix if prefix
        element_form_default element_form
        attribute_form_default attribute_form
      end
      end

      # Validate namespace class is proper subclass
      # Accepts both Lutaml::Xml::Namespace and Lutaml::Model::Xml::Namespace for compatibility
      def validate_namespace_class!(ns_class)
        is_valid = ns_class.is_a?(Class) && (
          (defined?(::Lutaml::Xml::Namespace) && ns_class < ::Lutaml::Xml::Namespace) ||
          (defined?(::Lutaml::Model::Xml::Namespace) && ns_class < ::Lutaml::Model::Xml::Namespace)
        )
        unless is_valid
          raise ArgumentError,
