      ...
      class NokogiriAdapter < Document
          ...
          if xml_mapping.no_element?
            ...
            if options[:tag_name]
              ...
              xml_mapping.elements.each do |element_rule|
                ...
                if value && extract_class(attribute_def, value)&.<=(Lutaml::Model::Serialize)
        ...
        def build_unordered_children_with_plan(xml, element, plan, options)
          ...
          mappings.each do |element_rule|
            ...
            if value && (extract_class(attribute_def, value)&.<=(Lutaml::Model::Serialize) || is_collection_instance)
              handle_nested_elements_with_plan(
                xml,
                value,
                element_rule,
        ...
        def build_ordered_element_with_plan(xml, element, plan, options)
          ...
          element.element_order.each do |object|
            ...
            if element_rule == xml_mapping.content_mapping
              ...
              if current_value && (extract_class(attribute_def, current_value)&.<=(Lutaml::Model::Serialize) || is_collection_instance)
                handle_nested_elements_with_plan(
                  xml,
                  current_value,
                  element_rule,
...
child_plan, options)
          element_options = options.merge(
            ...
            attribute: attribute,
            tag_name: rule.name,
            mapper_class: extract_class(attribute, value), # Override with child's type
          )
