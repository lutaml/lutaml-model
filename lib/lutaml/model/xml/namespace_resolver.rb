  ...
  module Model
    module Xml
      ...
      class NamespaceResolver
        ...
        def resolve_from_legacy_logic(rule, attribute, mapping, plan, options,
element_ns_uri, form_default)
          ...
          resolved_prefix = if rule.unqualified?
                              ...
                              nil
                            elsif rule.namespace_class && plan
                              ...
                              uri = rule.namespace_class.uri
                              ns_info_plan = DeclarationPlanQuery.find_namespace_by_uri(
                                plan, uri
                              )
                              if ns_info_plan && ns_info_plan[:format] == :prefix
                                ns_info_plan[:prefix]
                              else
                                ns_info[:prefix]
                              end
                            # Element has explicit namespace class - look it up in plan by URI
                            else
                              ns_info[:prefix]
                            end
