    ...
    module Xml
      module TypeNamespace
        ...
        class Planner
          ...
          def plan_for_element(mapping, mapper_class, element_namespace_class,
existing_declarations = {})
            ...
            resolved[:attributes].each do |ns_class|
              ...
              next unless @resolver.needs_declaration?(ns_class,
                                                       element_namespace_class)

              prefix = ns_class.prefix_default || generate_prefix("attr",
                                                                  existing_declarations)
            ...
            resolved[:elements].each do |ns_class|
              ...
              prefix = ns_class.prefix_default || generate_prefix("elem",
                                                                  existing_declarations)
          ...
          def plan_for_root(mapping, mapper_class, element_namespace_class)
            plan_for_element(mapping, mapper_class,
                             element_namespace_class).map do |decl|
          ...
          def generate_prefix(base, existing)
            counter = 0
            loop do
              prefix = counter.zero? ? "tn#{base}" : "tn#{base}#{counter}"
