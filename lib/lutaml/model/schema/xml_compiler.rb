...
begin
  require_relative "../../../xml/schema/xsd"
rescue LoadError
  # XSD compilation features will not be available
end
  ...
  module Model
    module Schema
      module XmlCompiler
        ...
        # Alias for easier access to XSD classes
        Xsd = Lutaml::Xml::Schema::Xsd if defined?(Lutaml::Xml::Schema::Xsd)

        # Only define methods if XSD module is available
        if defined?(Lutaml::Xml::Schema::Xsd)
...
compiler_complex_type)
          ...
          def resolved_element_order(object)
            ...
            if object.respond_to?(:resolved_element_order) &&
                object.class.name.start_with?("Lutaml::Xsd", "Lutaml::Xml::Schema::Xsd")
