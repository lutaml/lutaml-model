  ...
  module Model
    module Schema
      ...
      module Templates
        ...
        #   - Lutaml::Model::Schema::XmlCompiler::UnionSimpleType
        #   - Lutaml::Model::Schema::RngCompiler::UnionType
        # Each subclass provides `union_cast_body` with its specific
        # cast strategy (XSD: `||`-chained resolve_type; RNG: each+rescue).
        ...
        # Restriction facet sub-templates. Each emits one
        # `options[:facet] = ...` line. Callers gate emission with their
        # own predicate before evaluating. Consumed by:
        ...
        module Restriction
          MIN_MAX_BOUNDS = ERB.new(<<~TMPL, trim_mode: "-")
            <%= "\#{indent}options[:max] = \#{max_inclusive || max_exclusive}" if max_inclusive || max_exclusive %>
            <%= "\#{indent}options[:min] = \#{min_inclusive || min_exclusive}" if min_inclusive || min_exclusive %>
          TMPL

          PATTERN = ERB.new(<<~TMPL, trim_mode: "-")
            <%= indent %>options[:pattern] = %r{<%= pattern %>}
          TMPL

          ENUMERATIONS = ERB.new(<<~TMPL, trim_mode: "-")
