...
This follows the W3C XML Schema convention where local attributes are typically unqualified.
...
3. Is the attribute's type a Serializable model with its own namespace?
   YES → Extract namespace from the model's XML mapping
         (via attr_type.mappings_for(:xml, register_id).namespace_class)
...
[IMPORTANT]
.Serializable model namespaces come from their XML mapping
====
For `Serializable` model attributes, the namespace is stored on the model's
XML mapping rather than on the class itself. The rule compiler extracts it via
`attr_type.mappings_for(:xml, register_id).namespace_class`.

This means each distinct namespace variant of a same-named element must be
a separate model class. For example, if you have `<w14:docId>` and `<w15:docId>`
in the same document, you need two separate `Serializable` classes (`W14DocId`
and `W15DocId`), each with its own `namespace` declaration in its XML mapping.
====
...
Elements/attributes have no namespace (blank namespace):
...
[IMPORTANT]
.Explicit vs Default element_form_default
====
The distinction between explicitly setting `element_form_default` and relying on the
default is important:

* **Explicitly set** `element_form_default :unqualified`:
  - Children are placed in the blank namespace
  - Matches XSD's `elementFormDefault="unqualified"`
  - Predictable behavior during serialization

* **Not set** (defaults to `:unqualified`):
  - May not produce predictable unqualified behavior
  - Context-dependent inheritance may occur
  - Not recommended for XSD-compliant documents

**Always explicitly set** `element_form_default` to match your XSD.
====
