...
NOTE: The unified `xml do ... end` block API is consistent with Model classes.
The old `xml_namespace` directive at class level is deprecated but still supported.
...
.Overriding type namespace with child model
[example]
====
[source,ruby]
----
