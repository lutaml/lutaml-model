...
[[explicit-blank-namespace]]
==== Explicit blank namespace (`namespace :blank`)

When you need to explicitly specify that an element should have NO namespace
and prevent any namespace inheritance, use `namespace: :blank` in the mapping.

This is a more explicit alternative to `namespace: nil` that clearly indicates
"blank namespace" intent in your code.

.Mapping elements with explicit blank namespace
[example]
====
[source,ruby]
----
class MixedModel < Lutaml::Model::Serializable
  ...
  xml do
    ...
    namespace CeramicNamespace  # Has namespace
    map_element "Type", to: :type
    map_element "Note", to: :note, namespace: :blank  # Explicitly NO namespace
  end
end
----
...
NOTE: Both `namespace: nil` and `namespace: :blank` produce the same output.
Use `:blank` for clearer intent in code.
====
...
| 1 (HIGHEST)
| Explicit mapping level
| `map_element "name", to: :attr, namespace: SomeNamespace`
...
class Product < Lutaml::Model::Serializable
  ...
  xml do
    ...
    # Priority 1: Explicit mapping namespace (highest)
    map_element "name", to: :name, namespace: :blank
...
<item:product xmlns:item="http://example.com/items"
  ...
  <name>Widget</name>                    <!-- Priority 1: explicit :blank -->
