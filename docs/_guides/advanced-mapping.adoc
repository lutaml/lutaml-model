...
`when_attribute` rules sharing one key. A rule *without*
`when_attribute` is an unconditional claim: declaring one replaces every
prior rule on that key — including a parent class's rule the subclass
remaps to a different attribute. Last declaration wins for bare rules;
partition rules accumulate and replace only a predecessor with the same
target.
