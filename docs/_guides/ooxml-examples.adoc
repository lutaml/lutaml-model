...
== Example 3: Same-Named Elements in Different Namespaces

Word documents often contain elements with the same local name but different namespaces, such as `w14:docId` and `w15:docId`. Lutaml::Model handles this correctly during round-trip serialization.
...
Both `<w14:docId>` and `<w15:docId>` have the same local name `docId`. Without namespace-aware matching, serialization would lose one of them.
...
1. **Separate model classes**: Each namespace variant gets its own model class with its own namespace
2. **Namespace capture**: `element_order` captures the namespace URI for each element during parsing
3. **Namespace-aware matching**: Serialization matches elements by both name AND namespace URI
4. **Backward compatibility**: Name-only matching is used when namespace info is unavailable
...
When an OOXML document contains elements with the same local name but different namespace URIs (e.g. `w14:docId` and `w15:docId`), Lutaml::Model correctly distinguishes them during round-trip serialization.

This requires defining a separate `Serializable` model class for each namespace variant.
...
class WNamespace < Lutaml::Model::XmlNamespace
  uri "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
  prefix_default "w"
  element_form_default :qualified
end

class W14Namespace < Lutaml::Model::XmlNamespace
  uri "http://schemas.microsoft.com/office/word/2010/wordml"
  prefix_default "w14"
  element_form_default :qualified
end

class W15Namespace < Lutaml::Model::XmlNamespace
  uri "http://schemas.microsoft.com/office/word/2012/wordml"
  prefix_default "w15"
  element_form_default :qualified
end
...
# Settings model with two map_element entries sharing the same name
class Settings < Lutaml::Model::Serializable
  attribute :w14_doc_id, W14DocId
  attribute :w15_doc_id, W15DocId
  ...
  xml do
    ...
    # Each map_element maps to a distinct Serializable model type.
    # The namespace is picked up from the attribute's type, not from here.
    map_element "docId", to: :w14_doc_id
    map_element "docId", to: :w15_doc_id
  end
end
...
[source,ruby]
----
# Parse XML with both namespaced docId elements
xml = <<~XML
  <w:settings xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"
              xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml"
              xmlns:w15="http://schemas.microsoft.com/office/word/2012/wordml">
...
# Round-trip correctly serializes each element to its own namespace
serialized = settings.to_xml
reparsed = Settings.from_xml(serialized)
...
[IMPORTANT]
.Each namespace variant needs its own Serializable class
====
The namespace-aware matching works because each `map_element` targets an attribute of a distinct type (`W14DocId` vs `W15DocId`). The rule compiler extracts the namespace from each type's XML mapping, so the two `map_element "docId"` entries correctly serialize to `w14:docId` and `w15:docId` respectively.

If both attributes shared the same type, they would share the same namespace and one would overwrite the other.
====

=== Key Point: Serializable Model Namespaces

For `Serializable` model attributes, the namespace is stored on the model's XML mapping (accessible via `attr_type.mappings_for(:xml, register_id).namespace_class`), not directly on the type class. This is different from `Type::Value` subclasses which have `namespace_class` directly on the class.

The rule compiler handles this automatically, but understanding the distinction is important when debugging namespace-related issues.
...
* link:../xml-mappings-guide[XML Mappings Guide] for basic XML mapping
* link:../xml-namespace-qualification[XML Namespace Qualification] for detailed namespace control
* link:../../references/three-phase-namespace-architecture[Three-Phase Namespace Architecture] for implementation details
