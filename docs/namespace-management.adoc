...
=== The `prefix:` Serialization Option

When serializing a model to XML using `to_xml`, you can control how the root element's namespace is rendered using the `prefix:` option. This is a _serialization-time decision_ that allows the same model to output clean W3C-compliant XML (default namespace) or prefixed XML for legacy system compatibility.
...
| `nil` (not specified) - *Round-trip mode*
| Preserve input format during serialization. If the original XML used a prefix (e.g., `<oasis:table xmlns:oasis="urn:alias">`), the output will use a prefix. If the original XML used default namespace, the output will use default namespace. The original namespace URI (canonical or alias) is also preserved.

| `false`
| Force default namespace (xmlns="...") with no prefix on the element. Disables format preservation.

| `true`
| Force prefix format using `prefix_default` from the XmlNamespace class. Uses the namespace's defined default prefix (e.g., `"oasis"`).

| `:default`
| Explicitly force default namespace format (xmlns="...") with no prefix. Same as `false` but more semantic. Use this when you explicitly want default format override.

| `"string"`
| Use a custom prefix string (e.g., `"custom"`). Overrides both `prefix_default` and any input format. Useful when a legacy system requires a specific prefix.

|===

==== Round-trip Fidelity

For round-trip fidelity (parsing XML and re-serializing), the serializer preserves both the namespace format and the exact namespace URI from the original input:

* If the input XML used an alias URI (e.g., `urn:oasis:names:tc:xml:table`), the output will use that alias URI
* If the input XML used a canonical URI, the output will use the canonical URI
* The `prefix:` option only controls whether to use prefix format or default format, NOT which URI to use

[NOTE]
====
When using namespaces with `uri_aliases`, the system maintains a mapping between canonical URIs and their aliases. For example, a namespace might have:

* Canonical URI: `http://docs.oasis-open.org/ns/oasis-exchange/table`
* Alias URI: `urn:oasis:names:tc:xml:table`

When round-tripping, the original URI (alias or canonical) is preserved regardless of the `prefix:` option.
====
...
[source,ruby]
----
# Input: <oasis:table xmlns:oasis="urn:oasis:names:tc:xml:table">

# Round-trip: preserve input format (prefix + alias URI)
model.to_xml
# => <oasis:table xmlns:oasis="urn:oasis:names:tc:xml:table"/>

# Force prefix: use prefix_default but preserve alias URI
model.to_xml(prefix: true)
# => <oasis:table xmlns:oasis="urn:oasis:names:tc:xml:table"/>

# Force default: use default namespace but preserve alias URI
model.to_xml(prefix: :default)
# => <table xmlns="urn:oasis:names:tc:xml:table"/>

# Custom prefix: use custom prefix but preserve alias URI
model.to_xml(prefix: "custom")
# => <custom:table xmlns:custom="urn:oasis:names:tc:xml:table"/>
----
...
This section implements the following W3C XML Schema Databinding Patterns:

* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#UnqualifiedLocalElements[UnqualifiedLocalElements pattern]* (§2.1.2)
* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#QualifiedLocalAttributes[QualifiedLocalAttributes pattern]* (§2.1.3)
* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#ElementFormUnqualified[ElementFormUnqualified pattern]* (§2.7.4)
* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#AttributeFormQualified[AttributeFormQualified pattern]* (§2.8.1)

=== element_form_default

The `element_form_default` setting in XmlNamespace classes controls child element namespace qualification,
corresponding to W3C XML Schema's `elementFormDefault` attribute.

`:qualified`::: Children inherit parent namespace (W3C default for Lutaml::Model)
`:unqualified` (explicitly set)::: Children in blank namespace (requires xmlns="" in output)

**W3C Pattern: UnqualifiedLocalElements**
...
**W3C Pattern: ElementFormUnqualified**
...
====
For predictable behavior, always explicitly set `element_form_default` to match your
XSD's `elementFormDefault` attribute.

When `element_form_default :unqualified` is **explicitly set**:

* Children are placed in the blank namespace
* No namespace prefix on child elements
* Appropriate when XSD declares `elementFormDefault="unqualified"`

When `element_form_default` is **not set** (relies on default):

* May not produce predictable unqualified behavior
* Context-dependent inheritance may occur
* Not recommended for XSD-compliant serialization
====

.element_form_default behavior (explicitly set to :unqualified)
[example]
====
[source,ruby]
----
...
=== Declaration Plan Modes

The `import_declaration_plan:` option on `from_xml` controls how namespace
declarations are captured for round-trip fidelity:
...
| `:lazy`
| Yes
| Stores a lightweight reference to the parsed element tree. The declaration
  plan is built on first `to_xml` call, then cached. Best balance of fast
  parsing and round-trip fidelity.

| `:eager`
| No
| Builds the declaration plan during `from_xml`. Full round-trip fidelity
  with slightly slower parsing.

| `:skip`
| No
| Never builds the declaration plan. Fastest parsing, but namespace format
  (prefix vs default) is not preserved during `to_xml`.
|===

[source,ruby]
----
# Default (:lazy) — recommended for most use cases
model = MyClass.from_xml(xml)
model.to_xml  # Namespace format preserved

# Eager — for read-once-serialize-many workflows
model = MyClass.from_xml(xml, import_declaration_plan: :eager)

# Skip — for read-only parsing where serialization won't happen
...
Per the https://www.w3.org/TR/xml-names/[W3C XML Namespaces] specification,
namespace names are URI references (https://www.rfc-editor.org/rfc/rfc3986[RFC 3986]).

Both URLs and URNs are valid namespace identifiers:

* **URLs**: `http://example.com/schema`, `https://www.w3.org/2001/XMLSchema`
* **URNs**: `urn:ietf:params:xml:ns:vcard-4.0`, `urn:oasis:names:tc:xml:table`

NOTE: The W3C spec notes that conforming processors are *not required* to validate
that namespace names are syntactically valid URI references. Lutaml::Model uses
lenient validation to also accept non-standard identifiers (e.g., Formal Public
Identifiers like `-//OASIS//DTD XML Exchange Table Model//EN`).
...
NOTE: All adapters now produce identical namespace output. The Oga adapter namespace issues that existed in earlier versions have been resolved.
