...
* Collects `schema_location` from all XmlNamespace classes in scope
* Automatically adds XSI namespace to hoisted declarations when needed
* Emits `xsi:schemaLocation` attribute on appropriate elements
* Follows W3C XML Schema Part 1 specification
...
- Collects `schema_location` from all XmlNamespace classes in scope
- Automatically adds XSI namespace (`http://www.w3.org/2001/XMLSchema-instance`)
  to hoisted declarations when any namespace has a schema location
- Builds `xsi:schemaLocation` attribute value from namespace-location pairs
- Emits the attribute on the appropriate element (typically root)
