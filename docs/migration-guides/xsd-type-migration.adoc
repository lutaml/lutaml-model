= Migration Guide: XSD Type Declaration Architecture

[IMPORTANT]
====
**Status**: PROPOSAL for future implementation

**Current behavior**: Attribute-level `:xsd_type` option is documented in <<xsd-type-attribute-option>>

**Timeline**:

* v0.9.0: Deprecation warnings introduced
* v1.0.0: Attribute-level `:xsd_type` removed

**Tracking**: This document describes the proposed two-tier architecture to replace
...
[[model-type-naming]]
== Model Type Naming (Separate Concern)

Model complexType naming uses `type_name` (see <<type-name-method>>),
which is **completely separate** from attribute XSD type specification.
...
xml do
  ...
  type_name 'CustomTypeName'  # <1>
end
----
<1> Names the XSD complexType definition itself

Where,

`type_name`:: Sets the name of the XSD complexType or simpleType
definition. Default: `{ClassName}Type`. This has **nothing to do** with
attribute XSD types.

.Model type naming is orthogonal to attribute types
[example]
====
[source,ruby]
----
class Product < Lutaml::Model::Serializable
  attribute :product_id, IdType  # Attribute uses xs:ID type
  attribute :name, :string

  xml do
    element 'product'
    type_name 'CatalogItemType'  # ✅ complexType name (separate concern)

    map_attribute 'id', to: :product_id  # Uses xs:ID from IdType
    map_element 'name', to: :name        # Uses xs:string (inferred)
  end
end

# Generated XSD:
# <xs:element name="product" type="CatalogItemType"/>
#
# <xs:complexType name="CatalogItemType">  ← type_name
#   <xs:sequence>
#     <xs:element name="name" type="xs:string"/>
#   </xs:sequence>
#   <xs:attribute name="id" type="xs:ID"/>  ← xsd_type from IdType
# </xs:complexType>
----
====
...
class Product < Lutaml::Model::Serializable
  ...
  xml do
    map_attribute 'id', to: :id, xsd_type: 'xs:string'  # ❌ Not allowed
  end
end
# Should raise: Lutaml::Model::InvalidMappingArgumentError:
# xsd_type cannot be overridden at mapping level.
# XSD type is determined by Value type class.
...
The [`Type::Reference`](../../lib/lutaml/model/type/reference.rb) type
...
- [ ] Add `xsd_type` class method to [`Type::Value`](../../lib/lutaml/model/type/value.rb)
...
* <<xml-namespace-types>> for Value type namespaces
* <<type-name-method>> for Model complexType naming
* <<custom-types>> for Custom Value types
* Type registration documentation
* [`Type::Reference`](../../lib/lutaml/model/type/reference.rb)
