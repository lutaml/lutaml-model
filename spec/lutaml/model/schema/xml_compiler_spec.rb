...
RSpec.describe Lutaml::Model::Schema::XmlCompiler do
  describe ".to_models" do
    context "provided test valid xml schema, it should generate the models" do
      ...
      Dir.mktmpdir do |dir|
        it "generates the models, requires them, and tests them with valid and invalid xml" do
          ...
          expect(File.exist?("#{dir}/ct_math_test.rb")).to be_truthy
          expect(File.exist?("#{dir}/st_integer255.rb")).to be_truthy
          expect(File.exist?("#{dir}/long.rb")).to be_truthy
    ...
    describe "structure setup methods" do
      ...
      describe ".setup_sequence" do
        context "when given sequence contains sequence, element, choice, group" do
          let(:sequence) do
            sequence = Lutaml::Xsd::Sequence.new.tap do |ct|
              ct.sequence = [Lutaml::Xsd::Sequence.new(name: "test_sequence")]
              ct.element = [
          ...
          let(:expected_hash) { { sequences: [{}], elements: [{element_name: "test_element", type_name: nil}, {ref_class: "test_ref"}], choice: [{}], groups: [{}, {ref_class: "test_ref"}] } }
      ...
      describe ".setup_group_type" do
        context "when given group contains sequence, choice" do
          let(:group) do
            group = Lutaml::Xsd::Group.new.tap do |ct|
              ct.sequence = [Lutaml::Xsd::Sequence.new(name: "test_sequence")]
              ct.choice = [Lutaml::Xsd::Choice.new(name: "test_choice")]
      ...
      describe ".setup_extension" do
        context "when given extension contains attributes" do
          ...
          let(:expected_hash) { {extension_base: "test_base", attributes: [{base_class: "ST_Attr1", name: "Attr1", default: "1"}, {base_class: "ST_Attr2", name: "Attr2", default: "2"}], sequence: {}, choice: {} } }
