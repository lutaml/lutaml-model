...
RSpec.describe Lutaml::Model::Liquidatable do
  before do
    stub_const("DummyModel", Class.new(LiquidatableClass))
  end
  ...
  describe ".register_liquid_drop_class" do
    context "when drop class does not exist" do
      it "creates a new drop class" do
        expect { dummy.class.register_liquid_drop_class }
          .to change { dummy.class.const_defined?(:DummyModelDrop) }
          .from(false)
          .to(true)
      end
    end
    ...
    context "when drop class already exists" do
      it "raises an error" do
        ...
        expect { dummy.class.register_liquid_drop_class }
          .to raise_error(RuntimeError, "DummyModelDrop Already exists!")
      end
    end
  end
  ...
  describe ".register_drop_method" do
    ...
    it "defines a method on the drop class" do
      expect { dummy.class.register_drop_method(:display_name) }
        .to change { dummy.to_liquid.respond_to?(:display_name) }
        .from(false)
        .to(true)
    end
  end
