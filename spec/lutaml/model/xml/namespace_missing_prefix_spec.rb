...
RSpec.describe "Bug #8: Namespace Prefix Missing on First Element" do
  let(:table) { NamespaceMissingPrefixSpec::Table.new(id: "t1", content: "Cell") }
  let(:wrap) { NamespaceMissingPrefixSpec::TableWrap.new(table: table) }
  let(:article) { NamespaceMissingPrefixSpec::Article.new(table_wrap: wrap) }
  ...
  describe "serialization" do
    ...
    it "preserves prefix through round-trip serialization" do
      xml = article.to_xml
      parsed = NamespaceMissingPrefixSpec::Article.from_xml(xml)
      xml2 = parsed.to_xml
  ...
  describe "deserialization" do
    ...
    it "correctly parses prefixed elements" do
      parsed = NamespaceMissingPrefixSpec::Article.from_xml(input_xml)
  ...
  describe "nested elements in same namespace" do
    module NamespaceMissingPrefixSpecNested
      ...
      class Entry < Lutaml::Model::Serializable
        ...
        xml do
          element "entry"
          namespace NamespaceMissingPrefixSpec::OasisNamespace
      ...
      class Article < Lutaml::Model::Serializable
        ...
        xml do
          element "article"
          namespace_scope [NamespaceMissingPrefixSpec::OasisNamespace]
          map_element "table", to: :table
        end
      end
    end
    ...
    it "applies prefix to all nested elements in same namespace" do
      entry = NamespaceMissingPrefixSpecNested::Entry.new(text: "Cell")
      row = NamespaceMissingPrefixSpecNested::Row.new(entry: entry)
      table = NamespaceMissingPrefixSpecNested::TableNested.new(row: row)
      article = NamespaceMissingPrefixSpecNested::Article.new(table: table)
