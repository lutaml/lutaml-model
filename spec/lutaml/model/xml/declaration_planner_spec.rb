  ...
  describe "#plan" do
    context "with root element" do
      context "with default namespace" do
        ...
        let(:needs) do
          collector.collect(nil, mapping, mapper_class: model_class)
        end
    ...
    context "with nested elements" do
      context "when child namespace matches parent default (KEY TEST)" do
        ...
        let(:needs) do
          collector.collect(nil, mapping, mapper_class: contact_model)
        end
      ...
      context "when child namespace differs from parent" do
        ...
        let(:metadata_instance) do
          metadata_model.new(title: "Test", creator: "Author")
        end
        let(:document_instance) do
          document_model.new(version: "1.0", metadata: metadata_instance)
        end
        let(:needs) do
          collector.collect(document_instance, mapping,
                            mapper_class: document_model)
        end
    ...
    context "with type-only models (no_element)" do
      ...
      let(:needs) do
        collector.collect(nil, mapping, mapper_class: type_only_model)
      end
