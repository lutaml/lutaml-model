...
RSpec.describe "XML Encoding Handling" do
  ...
  if defined?(Lutaml::Model::Xml::OxAdapter)
    describe "with Ox adapter" do
      it_behaves_like "encoding behavior", Lutaml::Model::Xml::OxAdapter if TestAdapterConfig.adapter_enabled?(:ox)
    end
  end

  if defined?(Lutaml::Model::Xml::OgaAdapter)
    describe "with Oga adapter" do
      it_behaves_like "encoding behavior", Lutaml::Model::Xml::OgaAdapter if TestAdapterConfig.adapter_enabled?(:oga)
    end
  end
end