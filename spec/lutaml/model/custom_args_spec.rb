...
RSpec.describe "Passing state (custom args) in from_* and to_* methods" do
  ...
  if defined?(Lutaml::Xml::Adapter::NokogiriAdapter)
    context "with Nokogiri adapter" do
      it_behaves_like "with XML format", Lutaml::Xml::Adapter::NokogiriAdapter
    end
  end

  if defined?(Lutaml::Xml::Adapter::OgaAdapter) &&
      TestAdapterConfig.adapter_enabled?(:oga)
    context "with Oga adapter" do
      it_behaves_like "with XML format", Lutaml::Xml::Adapter::OgaAdapter
    end
  end
end
