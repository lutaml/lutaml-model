...
The `map_all` tag in XML mapping captures and maps all content within an XML element, including nested tags, attributes, and text nodes, into a single attribute in the target Ruby object. The `map_all` tag is exclusive and cannot be combined with other mappings (`map_element`, `map_attribute`, `map_content`) for the same element, ensuring it captures the entire inner XML content. An error is raised if `map_all` is defined alongside any other mapping in the same XML mapping context.
