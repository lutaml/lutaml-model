...
=== Reusable Classes with Import (Group)

Lutaml lets you create reusable element and attribute collections. These can be imported into other models using:

- `import_model`: imports both attributes and mappings (can be defined outside mappings block)
- `import_model_attributes`: imports only attributes (can be defined outside mappings block)
- `import_model_mappings`: imports only mappings (can be defined in mappings block or outside)

NOTE: In case of XML, reusable class must have `no_root`. Import order determines how elements and attributes are overwritten.
