...
see the **link:docs/index.adoc[📚 Documentation Index]**.
...
modeling needs. The adapter interface is also used to support certain transformations of models
...
* XML namespaces and mappings with `namespace: :blank` for explicit no-namespace elements
* Custom namespace prefixes during serialization through `to_xml(prefix:)` option
...
* Dynamically modify model attribute types using registers (see <<custom-registers>>)
  * Per-class default register for versioned schemas (e.g., MathML v2, v3)
...
[source,shell]
----
lutaml-model compare FILE1 FILE2 -m MODEL_FILE -r ROOT_CLASS
----
...
[source,shell]
----
lutaml-model compare data.xml data.json -m models/document.rb -r TermiumExtract
----
...
For detailed information and advanced usage examples, see link:docs/cli_compare.adoc[CLI Compare Documentation].
...
[source,ruby]
----
kiln = Kiln.new(brand: 'Acme', capacity: 100, temperature: 1050)

# Serialize to different formats (requires format mappings)
kiln.to_xml    # => XML string
kiln.to_json   # => JSON string
kiln.to_yaml   # => YAML string
kiln.to_toml   # => TOML string
----

==== Hash transformation

Hash transformation converts models to Ruby Hash objects without serialization to string format:

[source,ruby]
----
kiln = Kiln.new(brand: 'Acme', capacity: 100, temperature: 1050)

# Transform to Hash (no string serialization)
kiln.to_hash
# => {"brand" => "Acme", "capacity" => 100, "temperature" => 1050}

# Create model from Hash
Kiln.from_hash({"brand" => "Acme", "capacity" => 100, "temperature" => 1050})
# => #<Kiln:0x... @brand="Acme", @capacity=100, @temperature=1050>
----

NOTE: Hash transformation is different from serialization - it produces Ruby Hash objects rather than serialized string representations, making it useful for data transformation and testing scenarios.
...
For large schema libraries with many model classes, you can defer loading until
a type is actually needed by registering a **Proc** instead of a class. The Proc
is resolved on first `lookup`, and Ruby's `autoload` triggers the file load at
that point:
...
This pattern is especially useful in library entry points where you declare many
types but only a subset is used at runtime. It pairs naturally with Ruby's
built-in `autoload` for lazy file loading.

==== Handling special values

Custom types automatically handle special sentinel values via the
`UninitializedClassGuard` module. When `cast` is called:

* `nil` is passed through to your type's `cast` method
* `UninitializedClass.instance` is handled automatically - your type's `cast`
  method will never receive this sentinel value

This means you can write clean type implementations without needing to handle
...
class Email < Lutaml::Model::Type::String
  def self.cast(value)
    # nil check is optional - UninitializedClass is already filtered
...
[IMPORTANT]
====
If your `cast` method calls `super`, sentinel values are handled automatically.
You only need explicit handling if you override `cast` without calling `super`.
====
...
lutaml-model uses eager import resolution to resolve all imports at class definition time, avoiding runtime overhead.
...
class MyModel < Lutaml::Model::Serializable
  import_model_attributes :shared_attributes
  import_model_mappings :shared_mappings
end  # TracePoint :end fires - all imports resolved HERE

# Later: Uses pre-resolved mappings (no import overhead)
instance.to_xml
----

==== Two-Level Import System

**Attribute imports** (class-level):

* `import_model` - imports both attributes and mappings from another model
* `import_model_attributes` - imports only attributes
* `import_model_mappings` - imports only XML mappings

**XML mapping imports** (mapping-level):

* `import_model_mappings` inside xml block - imports element/attribute mappings

==== Graceful Fallback

If imports cannot be resolved at definition time (model not registered yet):

1. Import marked as incomplete
2. Resolution retried on first use (`attributes` or `mappings_for`)
3. Supports test setups where registration happens after definition

[example]
====
[source,ruby]
----
# Class defined (imports not resolvable yet)
class MyModel < Lutaml::Model::Serializable
  import_model :my_base  # Symbol stored
end

# Test setup registers the model
RSpec.describe MyModel do
  before do
    register.register_model(MyBase, id: :my_base)
  end

  it "resolves imports" do
    # First use triggers deferred resolution
    MyModel.attributes  # Resolves :my_base now
  end
end
----
====
...
Collections can define validation rules for their elements and for the collection as a whole.

===== Instance-level validation

Instance-level validations are applied to each individual item in the collection:
...
Collection-level validations are applied to the entire collection as a unit. These are useful for enforcing constraints that depend on the collection as a whole, such as uniqueness across items, collection size requirements, or business rules that involve multiple items.

====== Built-in collection validators
...
class BusinessRuleCollection < Lutaml::Model::Collection
  ...
  validate_collection do |publications, errors|
    ...
    if categories.length < 2
      errors.add(:collection, "Must have publications from at least 2 categories")
    end
  end
end
----
...
[source,ruby]
----
class ComprehensiveCollection < Lutaml::Model::Collection
  # Instance-level validations (for each item)
  instances(:publications, Publication) do
    validates :year, numericality: { greater_than: 1900 }
    validates :title, presence: true
  end
...
# Usage
collection = ComprehensiveCollection.new([...])
collection.validate!  # Runs both instance and collection validations
----
====
...
class PublicationCollection < Lutaml::Model::Collection
  ...
  # Validation blocks receive a context object as the third parameter
...
* `ctx[:key]` / `ctx[:key] = value` - Store and retrieve values
...
* `validates_uniqueness_of(field, message: nil)` - Ensures field values are unique across all items
* `validates_min_count(count, message: nil)` - Ensures minimum number of items in collection
* `validates_max_count(count, message: nil)` - Ensures maximum number of items in collection
* `validates_all_present(field, message: nil)` - Ensures all items have the specified field with a non-blank value
* `validate_collection(&block)` - Define custom validation logic (block can receive `collection, errors, ctx`)

See full documentation for collection validations at link:docs/collection_validation.adoc[Collection Validations].
...
Sets the URL where the XSD schema file can be found. When defined on a namespace
class, Lutaml::Model automatically emits the `xsi:schemaLocation` attribute in
the XML output following W3C XML Schema Part 1 specifications.
...
When any namespace in scope defines `schema_location`, the DeclarationPlanner:

. Adds the XSI namespace (`xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"`)
   to the root element's declarations
. Emits the `xsi:schemaLocation` attribute listing all namespace-location pairs
...
.Output XML (automatically generated):
[source,xml]
----
<Ceramic xmlns="https://example.com/schemas/ceramic/v1"
         xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
         xsi:schemaLocation="https://example.com/schemas/ceramic/v1
                             https://example.com/schemas/ceramic/v1/ceramic.xsd">
</Ceramic>
----
====

NOTE: The XSI namespace prefix is automatically managed. When multiple namespaces
define `schema_location`, all are included in a single `xsi:schemaLocation`
...
default, corresponding to W3C XML Schema's `elementFormDefault` attribute.

[IMPORTANT]
.Explicit vs Default Values
====
The distinction between explicitly setting `element_form_default` and relying on the
default value is significant:

* When **explicitly set** (e.g., `element_form_default :unqualified`): The value is
  propagated to child elements during serialization, causing them to be in the blank
  namespace (no xmlns attribute).

* When **not set** (defaults to `:unqualified`): The behavior depends on context.
  Programmatically created objects may inherit the parent's namespace, while parsed
  XML preserves the original format.

Always explicitly set `element_form_default` to match your XSD's `elementFormDefault`
attribute for predictable behavior.
====

W3C Compliance::

This directive implements the following W3C XML Schema Databinding patterns:

* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#UnqualifiedLocalElements[UnqualifiedLocalElements pattern]*
  - XSD: `elementFormDefault="unqualified"`
  - Instance: Local elements appear without namespace prefix

* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#ElementFormQualified[ElementFormQualified pattern]*
  - XSD: Local element with `form="qualified"`
  - Instance: Element appears with namespace prefix

* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#ElementFormUnqualified[ElementFormUnqualified pattern]*
  - XSD: Local element with `form="unqualified"`
  - Instance: Element appears without namespace prefix
...
class CeramicNamespace < Lutaml::Model::XmlNamespace
  uri 'https://example.com/schemas/ceramic/v1'
  prefix_default 'cer'
  element_form_default :qualified  # Explicitly set to match XSD
end
...
default, corresponding to W3C XML Schema's `attributeFormDefault` attribute.

[IMPORTANT]
.Explicit vs Default Values
====
Always explicitly set `attribute_form_default` to match your XSD's `attributeFormDefault`
attribute for predictable behavior.
====
...
* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#QualifiedLocalAttributes[QualifiedLocalAttributes pattern]*
  - XSD: `attributeFormDefault="qualified"`
  - Instance: Local attributes appear with namespace prefix

* *link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#AttributeFormQualified[AttributeFormQualified pattern]*
  - XSD: Local attribute with `form="qualified"`
  - Instance: Attribute appears with namespace prefix

[NOTE]
.Per W3C Conventions
====
Per W3C XML Schema conventions, attributes are typically unqualified (no namespace).
Link:https://www.w3.org/2002/ws/databinding/edcopy/advanced/advanced.html#QualifiedLocalAttributes[QualifiedLocalAttributes]
is required only when the XSD explicitly sets `attributeFormDefault="qualified"`.
====
...
Some legacy XML documents use Formal Public Identifiers (FPIs) as namespace URIs.
FPIs are SGML-era identifiers like `-//OASIS//DTD XML Exchange Table Model 19990315//EN`
and are NOT valid namespace URIs per the W3C XML Namespace specification. XML
namespaces must be absolute URIs.

Lutaml::Model can parse XML with FPI namespace URIs, but on serialization converts
them to valid URNs using the `urn:publicid:` namespace per
https://www.rfc-editor.org/rfc/rfc3151[RFC 3151]. For example:

* FPI: `-//OASIS//DTD XML Exchange Table Model 19990315//EN`
* URN: `urn:publicid:-//OASIS//DTD+XML+Exchange+Table+Model+19990315//EN`

NOTE: True round-trip is NOT possible for FPI namespaces because the input is
invalid XML. Lutaml::Model will produce valid XML output with the converted URN.

NOTE: DOCTYPE PUBLIC identifiers (used in `<!DOCTYPE>` declarations) are a
separate concept and are preserved correctly during round-tripping.

==== W3C standard namespace support

Lutaml::Model provides built-in support for W3C-defined standard namespaces:

* `Lutaml::Model::Xml::W3c::XmlNamespace` - W3C XML namespace (xml:lang, xml:space, xml:id, xml:base)
* `Lutaml::Model::Xml::W3c::XsiNamespace` - W3C XMLSchema-instance namespace (xsi:nil, xsi:type, xsi:schemaLocation)
* `Lutaml::Model::Xml::W3c::XlinkNamespace` - W3C XLink namespace (xlink:href, xlink:type, xlink:title, etc.)
* `Lutaml::Model::Xml::W3c::XsNamespace` - W3C XMLSchema namespace for XSD type definitions

These namespace classes adhere to W3C specifications and handle reserved prefixes correctly.
Lutaml::Model will warn if you attempt to define namespaces with W3C-reserved URIs or prefixes.

IMPORTANT: The `xml` prefix is implicitly bound per W3C specification and must NEVER be declared with `xmlns:xml`. Lutaml::Model automatically prevents this forbidden declaration.

.W3C XML namespace attribute types
[cols="1,2,2"]
|===
| Type Class | Symbol | Purpose
...
| `W3c::XmlSpaceType`
| `:xml_space`
| Whitespace handling ("default", "preserve")
...
| `W3c::XmlIdType`
| `:xml_id`
| Unique identifier (type ID)
|===

.W3C XSI namespace attribute types
[cols="1,2,2"]
|===
| Type Class | Symbol | Purpose
...
| `W3c::XsiNil`
| `:xsi_nil`
| Nil indicator ("true", "false")

| `W3c::XsiSchemaLocationType`
| `:xsi_schema_location`
| Schema location pairs

| `W3c::XsiNoNamespaceSchemaLocationType`
| `:xsi_no_namespace_schema_location`
| Schema location without namespace
|===

.W3C XLink namespace attribute types
[cols="1,2,2"]
|===
| Type Class | Symbol | Purpose
...
| `W3c::XlinkTypeAttrType`
| `:xlink_type`
| Link type ("simple", "extended", "locator", "arc", "resource", "title")

| `W3c::XlinkRoleType`
| `:xlink_role`
| Role URI

| `W3c::XlinkArcroleType`
| `:xlink_arcrole`
| Arc role URI
...
| `W3c::XlinkShowType`
| `:xlink_show`
| Display behavior ("new", "replace", "embed", "other", "none")

| `W3c::XlinkActuateType`
| `:xlink_actuate`
| Timing ("onLoad", "onRequest", "other", "none")
|===

.Using W3C XML namespace attributes (class-based)
[example]
====
[source,ruby]
----
...
puts doc.to_xml
# => <doc xml:lang="en" xml:space="preserve" xml:id="doc1">  Text with spaces  </doc>

# Note: No xmlns:xml declaration - it's implicitly bound per W3C specification
----
====

.Using W3C namespace attributes (symbol-based)
[example]
====
[source,ruby]
----
# W3C types are registered with Lutaml::Model::Type registry
# enabling convenient symbol-based access

class Link < Lutaml::Model::Serializable
  attribute :href, :xlink_href          # Symbol-based
  attribute :title, :xlink_title         # Symbol-based
  attribute :show, :xlink_show          # Symbol-based
...
puts link.to_xml
# => <Link xlink:href="http://example.com" xlink:title="Example" xlink:show="new"/>

# Symbol-based access also works for XSI types
class Item < Lutaml::Model::Serializable
...
See link:docs/_guides/xml-namespaces.adoc#w3c-standard-namespaces[W3C Standard Namespaces Guide] for comprehensive documentation.
...
Mixed content elements are whitespace-sensitive. Use the `xml_space` method
to automatically add `xml:space="preserve"` during serialization:
...
xml do
  ...
  xml_space :preserve  # Adds xml:space="preserve" to output
end
----

This signals to XML processors that whitespace in the element is significant.

.Using `mixed_content` with `xml_space`
[example]
====
[source,ruby]
----
class WhitespaceSensitiveText < Lutaml::Model::Serializable
  attribute :content, :string

  xml do
    element 'text'
    mixed_content        # Enables mixed content
    xml_space :preserve  # Auto-adds xml:space="preserve"
    map_content to: :content
  end
end

text = WhitespaceSensitiveText.new(content: "  Important spacing  ")
text.to_xml
...
NOTE: For comprehensive information on namespace declaration strategies including
the minimal-subtree principle and how `namespace_scope` grants ELIGIBILITY (not
automatic hoisting), see link:docs/_guides/xml-namespace-declarations.adoc[XML Namespace Declarations Guide].
The `namespace_scope` directive controls where namespace declarations
**are eligible to** appear in
...
NOTE: For comprehensive information on namespace_scope including child inheritance behavior, Type namespace interaction, and multi-namespace examples, see link:docs/namespace-management.adoc[XML Namespace Management Guide].
...
removed.
See the
link:docs/_migrations/0-9-0-namespace-api-migration.adoc[migration guide]
for details.
...
.Using `element_form_default :unqualified` for mixed qualification
[example]
====
When `element_form_default :unqualified`, child elements are not qualified
by default. To have qualified elements, create child model classes.
...
# Child model in different namespace
class Glaze < Lutaml::Model::Serializable
  attribute :value, :string
...
class Ceramic < Lutaml::Model::Serializable
  ...
  xml do
    ...
    map_element 'glaze', to: :glaze  # Uses GlazeNamespace (qualified)
  end
end
...
[[xml-raw-element]]
==== Capturing a specific element as raw XML (`raw_element`)

The `raw_element: true` option on `map_element` captures a specific mapped
element as a complete XML string, including its opening tag, attributes,
children, and closing tag.

This differs from `map_all` in two key ways:

* `map_all` captures the *entire inner XML* of the root element and is
  exclusive (no other mappings allowed).
* `raw_element` captures a *specific named element* and can coexist with
  other `map_element` and `map_attribute` mappings.

Use `raw_element: true` when you need to embed foreign XML vocabularies
(SVG, MathML, XSL-FO, etc.) within your model without parsing their
internal structure.
...
`raw_element: true` also works with collection attributes:
...
`raw_element` matches elements by local name regardless of namespace or prefix:
...
This is intentional — `raw_element` is designed to capture foreign XML
vocabularies without needing model classes for them.

===== Limitations

`raw_element` captures the element as a plain string.  If you need to
inspect or modify the captured XML (e.g., filter attributes, transform
children), use a wrapper model with `map_all` instead:
...
==== Mapping processing instructions (`map_processing_instruction`)

Processing Instructions (PIs) are XML nodes of the form `<?target content?>`
that appear before the root element. They are used by XML applications to pass
application-specific information.

Common use cases include:

* xml2rfc PIs: `<?rfc strict="yes"?>`, `<?rfc compact="yes"?>`
* xml-stylesheet PIs: `<?xml-stylesheet href="style.xsl"?>`
* DocBook PIs: `<?db toc="yes"?>`

===== Hash mode (default)

When the mapped attribute is a `:hash`, each PI matching the target generates
one key-value pair. Multiple PIs with the same target are merged into a single
hash.

This mode is extensible -- adding a new PI key does not require model changes.
...
----
xml do
  map_processing_instruction "target_name", to: :attribute_name
end
----

.Mapping xml2rfc processing instructions to a hash attribute
[example]
====
[source,ruby]
----
class RfcDoc < Lutaml::Model::Serializable
  attribute :title, :string
...
[source,ruby]
----
> doc = RfcDoc.from_xml('<?rfc strict="yes"?><?rfc compact="no"?><rfc><title>Test</title></rfc>')
> doc.pi_settings
# => {"strict"=>"yes", "compact"=>"no"}

> doc.pi_settings = { "strict" => "yes", "compact" => "yes" }
> doc.to_xml
# => <?rfc strict="yes"?><?rfc compact="yes"?><rfc><title>Test</title></rfc>
----
====

===== Array mode

When the mapped attribute is a `:string` collection, each PI's raw content
string is stored as-is.

.Mapping processing instructions to a string collection (array mode)
[example]
====
[source,ruby]
----
...
Multiple `map_processing_instruction` calls map different PI targets to
...
* Only document-level PIs (before the root element) are supported.
* Inline PIs within element content are not yet supported (requires
  `mixed_content` integration).
...
This provides consistency with Model classes (which use the same `xml do ... end` block).
...
----
<1> The `xml` block provides unified XML configuration
...
| `nil` (not specified)
| *Round-trip mode* (default): Preserve input format. If the original XML used a prefix, the output will use a prefix. If the original XML used default namespace, the output will use default namespace.

| `false`
| Force default namespace (xmlns="...") with no prefix on the element. Disables format preservation.

| `true`
| Force prefix format using `prefix_default` from the XmlNamespace class. Uses the namespace's defined default prefix.

| `:default`
| Explicitly force default namespace format (xmlns="...") with no prefix. Same as `false` but more semantic.

| `"string"`
| Use a custom prefix string (e.g., `"custom"`). Overrides both `prefix_default` and any input format.
...
[NOTE]
====
*Round-trip fidelity:* When no `prefix:` option is specified (or `nil`), the serializer preserves the original namespace format from parsed input. The original namespace URI (canonical or alias) is also preserved for systems that use URI aliases.

*URI preservation:* When using `prefix: true`, `prefix: :default`, or `prefix: "custom"`, the original namespace URI (alias or canonical) from the input is still preserved for round-trip fidelity. The `prefix:` option only controls the *format* (prefix vs default), not the *URI*.
====
...
[[form-override]]
==== Form override for element qualification

The `form:` option provides per-element control over W3C XML Schema qualification, allowing you to override namespace defaults on individual elements or attributes.

===== General

Per W3C XML Schema specification, the `form` attribute controls whether elements must be qualified (namespace-prefixed) or unqualified (no prefix), independent of the schema's `elementFormDefault` setting.

Form override takes **highest priority** in namespace resolution, above type namespaces, inheritance, and schema defaults.
...
xml do
  ...
  map_attribute 'id', to: :id, form: :qualified     # Force prefix (unusual)
end
----

Values:

`:qualified`::: Element **MUST** use namespace prefix, even if schema default is unqualified
`:unqualified`::: Element **MUST NOT** use namespace prefix, even if parent has namespace

===== Priority order

Form takes highest priority in namespace resolution:
...
===== Example: Mixed qualification

.Using form to mix qualified and unqualified elements
[example]
====
[source,ruby]
----
class ProductNamespace < Lutaml::Model::XmlNamespace
  ...
  prefix_default "prod"
  element_form_default :qualified  # Default: all elements qualified
end

class Product < Lutaml::Model::Serializable
  attribute :id, :string
  attribute :name, :string
  attribute :internal_code, :string

  xml do
    element "product"
    namespace ProductNamespace

    map_element "id", to: :id, form: :qualified          # Uses prefix (follows default)
    map_element "name", to: :name, form: :qualified      # Uses prefix (follows default)
    map_element "code", to: :internal_code, form: :unqualified  # NO prefix (override)
  end
end

product = Product.new(id: "P123", name: "Widget", internal_code: "INTERNAL-001")
puts product.to_xml(prefix: true)
----
...
<prod:product xmlns:prod="http://example.com/products">
  <prod:id>P123</prod:id>
  <prod:name>Widget</prod:name>
  <code>INTERNAL-001</code>  <!-- No prefix, local scope -->
</prod:product>
----

NOTE: The `code` element is unqualified despite the schema's `element_form_default: :qualified`. The `form: :unqualified` override forces local scope.
====

===== Use cases

**Use `form: :qualified` when:**

* Schema specifies `elementFormDefault="unqualified"` but specific elements need qualification
* Ensuring element is unambiguous across namespace boundaries
* Legacy system requires specific elements to be prefixed

**Use `form: :unqualified` when:**

* Schema specifies `elementFormDefault="qualified"` but specific elements should be local
* Element represents internal data not part of public schema
* Mixing qualified and unqualified elements in same namespace
...
class Document < Lutaml::Model::Serializable
  ...
  map_element 'title', to: :title  # Uses Title's namespace (DublinCoreNamespace)
end
----
...
=== XML Namespace Format Preservation
...
When parsing XML with a default namespace declaration, Lutaml::Model automatically preserves this format on serialization:

.Parsing and preserving default namespace format
[example]
====
[source,xml]
----
<TestModel xmlns="http://example.com">
  <name>Test</name>
</TestModel>
----
...
model.to_xml  # Outputs with xmlns="..." (default format)
----
====

==== Prefix namespace format

When parsing XML with a prefixed namespace declaration, Lutaml::Model automatically preserves this format on serialization:

.Parsing and preserving prefix namespace format
[example]
====
[source,xml]
----
<test:TestModel xmlns:test="http://example.com">
...
----
model = TestModel.from_xml(xml)
model.to_xml  # Outputs with xmlns:test="..." (prefix format)
----
====

==== Programmatic control

For models created programmatically (not from XML), use the `prefix` option to control namespace format:

.Controlling namespace format programmatically
[example]
====
[source,ruby]
----
...
Format preservation uses a lazy declaration plan by default. When you call `from_xml`,
the parsed element tree is stored as a lightweight reference. On first `to_xml`, the
declaration plan is built from this reference and cached for subsequent serializations,
using the three-phase namespace architecture (Collector → Planner → Builder).

You can control this behavior with the `import_declaration_plan:` option:

[source,ruby]
----
# Default: lazy — plan built on first to_xml (recommended)
model = MyClass.from_xml(xml)

# Eager: build plan during parsing
model = MyClass.from_xml(xml, import_declaration_plan: :eager)

# Skip: never build plan (fastest, no round-trip fidelity)
model = MyClass.from_xml(xml, import_declaration_plan: :skip)
----

For more details, see link:docs/_guides/xml-namespaces.adoc[XML Namespaces Guide].

=== Same-Named Elements in Different Namespaces

When an XML document contains elements with the same local name but different namespace URIs (common in OOXML documents), Lutaml::Model correctly distinguishes them during round-trip serialization.

==== The Problem

In OOXML documents, you may encounter:
...
<w:settings xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"
  ...
  <w15:docId w15:val="{A1B2C3D4}"/>
</w:settings>
----

Both `<w14:docId>` and `<w15:docId>` have the same local name `docId` but different namespaces. Without namespace-aware matching, serialization would lose one of them.

==== The Solution

Lutaml::Model captures namespace information during parsing and uses it during serialization to correctly match elements to their mapping rules.

.Supporting Same-Named Elements
[example]
====
This requires defining separate model classes for each namespace variant:
...
# Define namespaces
class W14Namespace < Lutaml::Xml::W3c::XmlNamespace
  uri "http://schemas.microsoft.com/office/word/2010/wordml"
  prefix_default "w14"
  element_form_default :qualified
end

class W15Namespace < Lutaml::Xml::W3c::XmlNamespace
  uri "http://schemas.microsoft.com/office/word/2012/wordml"
  prefix_default "w15"
  element_form_default :qualified
end

class WNamespace < Lutaml::Xml::W3c::XmlNamespace
...
class W15DocId < Lutaml::Model::Serializable
  ...
  xml do
    element "docId"
    namespace W15Namespace
    map_attribute "val", to: :val
  end
end

# Settings model with same-named elements
class Settings < Lutaml::Model::Serializable
  ...
  xml do
    ...
    map_element "chartTrackingRefBased", to: :chart_tracking
...
# Parse - both elements are correctly captured
settings = Settings.from_xml(xml)
settings.w14_doc_id.val   # => "4C6D0839"
settings.w15_doc_id.val   # => "{A1B2C3D4}"

# Round-trip preserves both elements
serialized = settings.to_xml
reparsed = Settings.from_xml(serialized)

reparsed.w14_doc_id.val   # => "4C6D0839" (preserved!)
reparsed.w15_doc_id.val   # => "{A1B2C3D4}" (preserved!)
----
====

==== How It Works

1. **Parsing**: `XmlElement#order` captures namespace URI and prefix for each child element
2. **Matching**: `OrderedApplier#find_rule_for_element` matches by both name AND namespace URI when available
3. **Namespace extraction**: For `Serializable` model attributes (e.g. `attribute :w14_doc_id, W14DocId`), the namespace is automatically extracted from the model's own XML mapping, not from the parent. This ensures correct round-trip serialization even when multiple `map_element` entries share the same local name but target different namespaces.
4. **Fallback**: Name-only matching is used for backward compatibility with existing code

[IMPORTANT]
.Separate model classes are required
====
Each distinct namespace variant of a same-named element **must** have its own `Serializable` model class. The namespace-aware matching works by pairing the `map_element` rule with its target attribute's type. If two attributes share the same type, they will share the same namespace.

For example, `attribute :w14_doc_id, W14DocId` and `attribute :w15_doc_id, W15DocId` must be distinct classes (e.g. `W14DocId` and `W15DocId`) so that each `map_element` rule picks up the correct namespace from its attribute's type.
====

For more details on OOXML patterns, see link:docs/_guides/ooxml-examples.adoc[OOXML Examples].
...
Lutaml::Model uses a symmetric object-oriented architecture for all serialization
formats. This unified approach provides clear separation of concerns and
eliminates the need for primitive Hash objects during transformation.

**Symmetric architecture across formats:**

[source]
----
XML Serialization:
  Model → Xml::Transformation → XmlDataModel → Adapters → XML String

Key-Value Serialization (JSON/YAML/TOML):
  Model → KeyValue::Transformation → KeyValueElement → Adapters → Format String
----

This architecture provides:

Content/Presentation Separation::
KeyValueElement defines WHAT to serialize (content layer), while adapters define
HOW to serialize it (presentation layer)

OOP Throughout::
No primitive Hash usage during model transformation ensures type safety and
clear interfaces

Extensibility::
Easy to add new formats or customize behavior without modifying existing code
...
The `KeyValueElement` class is the intermediate representation for key-value
formats (JSON, YAML, TOML). It provides an object-oriented container for:

* Element name (key)
* Element value (string, array, or nested KeyValueElement)
...
element = Lutaml::KeyValue::DataModel::Element.new(
  ...
  nil,  # No direct value
  [
    Lutaml::KeyValue::DataModel::Element.new("name", "John Doe"),
    Lutaml::KeyValue::DataModel::Element.new("age", "30")
  ]
)
----
...
All JSON, YAML, and TOML adapters accept `KeyValueElement` objects with
backward compatibility for legacy Hash-based serialization.

**Adapter compatibility:**

* `Json::StandardAdapter`, `Json::MultiJsonAdapter`, `Json::OjAdapter`
* `Yaml::StandardAdapter`
* `Toml::TomlRbAdapter`, `Toml::TomlibAdapter`

The adapters automatically detect whether they receive a KeyValueElement or
legacy Hash structure and handle accordingly.

=== Comparison with XmlDataModel

Both systems follow the same architectural pattern:

[cols="1,2,2"]
|===
| Aspect | XmlDataModel | KeyValueDataModel

| Purpose
| Intermediate representation for XML
| Intermediate representation for JSON/YAML/TOML

| Content Layer
| `XmlElement` with namespace_class
| `KeyValueElement` with name/value/children
...
| Benefits
| Namespace planning, W3C compliance
| Clean OOP structure, extensible

|===

For detailed documentation on KeyValueDataModel including transformation
compilation, adapter integration, and extension points, see
link:docs/key_value_data_model.adoc[KeyValueDataModel Architecture Guide].
...
serialization into key-value formats including JSON, YAML, and TOML, as well as
transformation to Hash format.

If there is no need to differentiate among the key-value formats (JSON, YAML, TOML) and Hash transformation, the
...
data model (JSON, YAML, TOML serialization formats, or Hash transformation) into a `Lutaml::Model::Serializable`
...
==== Module namespace for generated models

The `module_namespace` option allows organizing generated models into a Ruby
module namespace, enabling:

* **Multi-version schema support**: Different schema versions coexist without
naming conflicts
* **Namespace isolation**: Models from different schemas remain separate
* **Clean code organization**: Generated models grouped logically
...
Lutaml::Model::Schema::XmlCompiler.to_models(
  xsd_content,
  module_namespace: "ModuleName",  # <1>
  register_id: :register_symbol,    # <2>
  output_dir: "lib/module_dir",     # <3>
  create_files: true                # <4>
)
----
<1> Module namespace for generated classes (e.g., `"UnitsMLV0919"`)
<2> Register ID for model registration (required with `module_namespace`)
<3> Output directory (auto-generates module namespace if not provided)
<4> Write files to disk

.Compiling UnitsML v0.9.19 schema with module namespace
[example]
====
[source,ruby]
----
...
# Generate models in UnitsMLV0919 module namespace
Lutaml::Model::Schema::XmlCompiler.to_models(
  xsd_content,
...
# Generated files:
# lib/unitsml/unitsmlv0919_registry.rb - Central registry
# lib/unitsml/unit_type.rb              - Model files
# lib/unitsml/quantity_type.rb
# ...

# Load all models via registry:
require "unitsml/unitsmlv0919_registry"
UnitsMLV0919.register_all

# Use the namespaced models:
doc = UnitsMLV0919::UnitsMLType.from_xml(xml)
doc.class  # => UnitsMLV0919::UnitsMLType
----
====

The central registry file (`{module_name}_registry.rb`) provides:

* **Autoload pattern**: Lazy-load models as needed
* **Centralized registration**: Single `register_all` method
* **Thread-safe**: Uses `@register` instance variable per class

.Generated registry file structure
[source,ruby]
----
module UnitsMLV0919
  autoload :UnitsMLType, "unitsml/units_ml_type"
  autoload :UnitType, "unitsml/unit_type"
  # ... other models

  def self.register_all
    register = Lutaml::Model::Register.new(:unitsmlv0919)
    ...
    # Set @register on each class
    [UnitsMLType, UnitType, ...].each do |klass|
      klass.instance_variable_set(:@register, register)
    end
  end
end
----

For comprehensive information on using module namespaces with the schema
compiler, see
link:docs/_tutorials/schema-compiler-module-namespaces.adoc[Schema Compiler
Module Namespaces Tutorial].

==== Register fallback mechanism

Custom registers support **hierarchical fallback**, allowing specialized schemas
to reuse types from foundation schemas without duplication.

By default, all custom registers fall back to the `:default` register. This
enables:

* **Shared types**: Common types like `annotation`, `documentation` are resolved
from the default register
* **Isolation option**: Registers can disable fallback for strict schema isolation
* **Custom chains**: Multi-level fallback for schema profiles

Syntax:

[source,ruby]
----
# Default behavior: fall back to :default
register = Lutaml::Model::Register.new(:my_schema)
register.fallback  # => [:default]

# Isolated register: no fallback
isolated = Lutaml::Model::Register.new(:pristine, fallback: [])
isolated.fallback  # => []

# Custom fallback chain: :profile → :core → :default
...
.Using register fallback for schema profiles
[example]
====
[source,ruby]
----
...
core_register.register_model(GmlCoreType, id: :gml_core_type)
core_register.register_model(AnnotationType, id: :annotation)

# Specialized profile extends core types
...
profile_register.register_model(GmlProfileType, id: :gml_profile_type)

# Profile models can use both profile and core types
class ProfileDocument < Lutaml::Model::Serializable
  @register = profile_register

  attribute :profile_data, :gml_profile_type  # From :gml_profile
  attribute :annotation, :annotation           # From :gml_core (fallback)
end
----
====

For comprehensive information on register patterns including vertical separation,
horizontal separation, and version coexistence, see
link:docs/_references/custom_registers.adoc[Custom Registers Guide].
...
fault issues. On Windows, the `toml-rb` adapter is used automatically as the
default. If you explicitly configure `tomlib` on Windows, a
...
1. *<<xsd-schema-parsing,XSD Schema Parsing>>* - Parse XSD schemas into Ruby
   objects for inspection, manipulation, and round-tripping.
2. *<<schema-import,Schema import>>* (this section) - Generate Ruby model
   classes from XSD schemas that can be used for serialization.

Use XSD Schema Parsing when you need to work with the schema itself.
Use Schema import when you want to create serializable Ruby classes from XSD.
...
Lutaml::Model provides functionality to parse XSD (XML Schema Definition)
schemas into Ruby objects for inspection, manipulation, and round-tripping.
This allows you to work with XSD schemas programmatically without generating
Ruby model classes.
...
NOTE: This feature is distinct from <<schema-import,Schema import>> which
generates Ruby model classes from XSD schemas. XSD Schema Parsing is for
working with XSD schemas as objects.

[[xsd-basic-parsing]]
=== Basic parsing

To parse an XSD schema into Ruby objects, use the `Lutaml::Xml::Schema::Xsd.parse`
method.

[source,ruby]
----
require "lutaml/xml/schema/xsd"  # <1>

schema = Lutaml::Xml::Schema::Xsd.parse(xsd_content)  # <2>

schema.complex_type.each do |type|  # <3>
...
`require "lutaml/xml/schema/xsd"`::: Loads the XSD parsing module and all
required model classes.
`Lutaml::Xml::Schema::Xsd.parse(xsd_content)`::: Parses the XSD content string
and returns a `Schema` object representing the root `xs:schema` element.
`schema.complex_type`::: Accesses the collection of complex type definitions in
the schema.

.Parsing an XSD schema and accessing its components
[example]
====
[source,ruby]
----
...
puts schema.target_namespace    # => "http://example.com/test"
puts schema.element_form_default # => "qualified"

# Access complex types
schema.complex_type.each do |type|
  puts "Complex type: #{type.name}"
  type.elements.each { |e| puts "  - Element: #{e.name}" }
end

# Access elements
schema.element.each do |elem|
  puts "Global element: #{elem.name}, type: #{elem.type}"
end
...
The `parse` method accepts several options to control parsing behavior:
...
`location`::: Base path or URL for resolving relative paths in `xs:include` and
`xs:import` statements. When provided, referenced schemas are automatically
loaded and merged into the main schema.

`schema_mappings`::: Array of `SchemaLocationMapping` objects to redirect schema
locations. Useful when working with local copies of remote schemas.

`validate_schema`::: When `true` (default), validates the XSD structure before
parsing using `SchemaValidator`. Set to `false` to skip validation for
malformed schemas that you want to parse anyway.

`nested_schema`::: Internal flag used when recursively parsing included/imported
schemas. Should not be set by users.

`register`::: Custom type register to use for parsing. Defaults to the built-in
`:xsd` register.

.Using location option to resolve includes
[example]
====
[source,ruby]
----
...
# Parse schema with base location for resolving includes
schema = Lutaml::Xml::Schema::Xsd.parse(
  File.read("schemas/main.xsd"),
  location: "schemas/"
)

# Included schemas are automatically parsed and merged
puts "Total complex types: #{schema.complex_type.size}"
----
====

[[xsd-working-with-schemas]]
=== Working with parsed schemas

The `Schema` object provides several methods for accessing and querying schema
components.
...
----
# Access collections
schema.element           # Array of global element declarations
schema.complex_type      # Array of complex type definitions
schema.simple_type       # Array of simple type definitions
schema.attribute         # Array of global attribute declarations
schema.group             # Array of named model groups
schema.attribute_group   # Array of attribute groups
schema.import            # Array of xs:import statements
schema.include           # Array of xs:include statements
...
[source,ruby]
----
# Find a type definition by name (searches both simple and complex types)
type = schema.find_type("AddressType")

# Find a specific complex type
complex = schema.find_complex_type("AddressType")

# Find a specific simple type
simple = schema.find_simple_type("StatusType")

# Find a global element by name
...
# Get statistics hash
stats = schema.stats
# => {:elements=>1, :complex_types=>1, :simple_types=>0, ...}

# Basic validation check (has target namespace)
puts schema.valid? # => true/false

# Get schema name derived from target namespace
puts schema.name # => "test" (from "http://example.com/test")
----

==== Round-tripping

Parsed schemas can be serialized back to XML:
...
# Navigate into complex type
person_type = schema.find_complex_type("PersonType")

# Access sequence content
sequence = person_type.sequence
puts "Sequence has #{sequence.element.size} direct elements"

# Access nested choice
choice = sequence.choice.first
puts "Choice between: #{choice.element.map(&:name).join(", ")}"

# Access attributes
person_type.attribute.each do |attr|
  puts "Attribute: #{attr.name}, use: #{attr.use}"
end
----
====
...
Before parsing, XSD content can be validated using `SchemaValidator`:
...
validator = Lutaml::Xml::Schema::Xsd::SchemaValidator.new(version: "1.0")
puts validator.validate(valid_xsd)  # => true

# Invalid: wrong root element
invalid_xsd = "<notASchema/>"
begin
  validator.validate(invalid_xsd)
rescue => e
  puts "Error: #{e.message}"
  # => "Not a valid XSD schema: root element must be 'schema', found 'notASchema'"
end
----
====

[[xsd-supported-elements]]
=== Supported XSD elements

The XSD parser supports the following XSD constructs:
...
Parsed XSD objects expose helper methods designed for use with Liquid
templates (see <<liquid-template-access>>).  These helpers resolve
cross-references, flatten nested content models, and provide sorted
access to schema components.
...
.Base-level helpers available on all XSD model objects
[cols="1,4"]
|===
| Method | Description
...
For generating Ruby model classes from XSD schemas, use the
<<schema-import,Schema import>> feature instead.
...
Registers manage models and type resolution in a format-agnostic way. The
namespace binding mechanism specifically works with `Lutaml::Xml::Namespace` subclasses:
bind XML namespace classes to a register to enable version-aware type resolution.

[source,ruby]
----
# Bind Xml::Namespace subclasses to a register
register.bind_namespace(XmiNamespace)
register.handles_namespace?("http://example.com/ns/20131001")  # => true

# Resolve type with namespace awareness
klass = register.resolve_in_namespace(:documentation, "http://example.com/ns/20131001")
----

This enables *version-aware type resolution* where the same type name resolves to
different classes depending on the XML namespace URI in the document. See
link:docs/_references/custom_registers.adoc#namespace-bound-registers[Namespace-Bound Registers]
in the Custom Registers Guide for a complete example with fallback chains and
mixed-namespace document handling.

[[global-context-api]]
==== GlobalContext API (v0.7+)

[IMPORTANT]
====
The `GlobalContext` API is the recommended approach for new applications.
It provides a cleaner, more object-oriented architecture with centralized
state management and better test isolation.

The legacy `Register` and `GlobalRegister` APIs are deprecated but remain
fully functional for backward compatibility.
====

The `GlobalContext` class provides a single entry point for all mutable global
state, following SOLID principles with proper separation of concerns.
...
| Access the ContextRegistry (named context store)

| `GlobalContext.resolver`
| Access the CachedTypeResolver (type resolution with caching)

| `GlobalContext.imports`
| Access the ImportRegistry (deferred import management)

| `GlobalContext.default_context`
| Get the current default TypeContext

| `GlobalContext.resolve_type(name)`
| Resolve a type name to a class using default context

| `GlobalContext.create_context(id:, substitutions:)`
| Create and register a new TypeContext

| `GlobalContext.with_context(context_id) { ... }`
...
.Using GlobalContext for custom type substitution
[example]
====
[source,ruby]
----
# Create a context with custom type substitution
context = Lutaml::Model::GlobalContext.create_context(
  id: :my_app,
  substitutions: [
    Lutaml::Model::TypeSubstitution.new(
      from_type: Lutaml::Model::Type::DateTime,
      to_type: MyCustomDateTime
    )
  ]
)
...
.Test isolation with GlobalContext
[example]
====
[source,ruby]
----
...
RSpec.configure do |config|
  config.before do
    # ONE method clears ALL global state
