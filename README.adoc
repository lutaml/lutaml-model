...
`raw_element` matches elements by unprefixed local name, so it captures:
...
Elements with an explicit namespace *prefix* (`<ns:svg>`) are not matched by
`raw_element`, because namespace-qualified matching requires a model type with
the corresponding namespace class.  For such elements, create a wrapper model
using `map_all` instead:
...
class Container < Lutaml::Model::Serializable
  ...
  xml do
    root "container"
    map_element "svg", to: :svg_data   # matches <ns:svg> via type namespace
  end
end
----
====
