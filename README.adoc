...
using the `collection` option. We can set `collection` to `true` or set a range
like `collection: 5..10` this will raise `CollectionCountOutOfRangeError` if
the count of values for the field is less then 5 or greater then 10.
...
attribute :name_of_attribute, Type, collection: 0..5
attribute :name_of_attribute, Type, collection: 0..  # This means 0 to Infinity
----
