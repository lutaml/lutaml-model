...
=== Value Map Functionality in Lutaml Model

The `value_map` functionality allows users to specify a mapping between the values of an attribute in the model and the values that will be serialized. This mapping is used to convert values between the Ruby object and the serialized format.

==== Overview

The `value_map` option can be used to define mappings for both `from` and `to` values:

- `from` values: Used when reading data from a format into Ruby objects.
- `to` values: Used when converting Ruby objects back into the serialized format.

Users can specify the mapping for both `from` and `to` values using the `value_map` option in the attribute definition. The keys that can be used in the `from` and `to` mappings are `empty`, `omitted`, and `nil`. The values in the mappings can also be `empty`, `omitted`, and `nil`.

However, note that `nil` is not supported in TOML, so mappings like `nil: any_option` or `any_option: nil` will not work in TOML.
...
The following table summarizes the default `value_map` configurations for various serialization formats. Each format has specific behavior when handling values such as `empty`, `omitted`, and `nil`.

[cols="1,1,1,1", options="header"]
|===
| Format      | `from` Value  | `to` Value      | Description

| XML         | `empty`       | `nil`           | An empty value is treated as `nil` during serialization.
| XML         | `omitted`     | `omitted`       | If the value is omitted in the model, it remains omitted in the serialized output.
| XML         | `nil`         | `nil`           | A `nil` value in the model is serialized as `nil`.

| YAML        | `empty`       | `empty`         | An empty string (`""`) in the model is deserialized as `empty`.
| YAML        | `omitted`     | `omitted`       | If the value is omitted, it remains omitted in YAML.
| YAML        | `nil`         | `nil`           | A `nil` value in the model is serialized as `nil`.

| JSON        | `empty`       | `empty`         | An empty string (`""`) in the model is serialized as `empty`.
| JSON        | `omitted`     | `omitted`       | If the value is omitted in the model, it remains omitted in the JSON output.
| JSON        | `nil`         | `nil`           | A `nil` value in the model is serialized as `null`.

| TOML        | `empty`       | `empty`         | An empty string (`""`) in the model is treated as `empty` for TOML serialization.
| TOML        | `omitted`     | `omitted`       | If the value is omitted in the model, it remains omitted in the TOML output.
| TOML        | `nil`         | `omitted`       | In TOML, `nil` is represented as `omitted` due to TOML limitations.
|===
...
== Example Usage with Collections
