...
You can also define **one custom method** (`from` or `to`) inside the `with` block. In this case, ensure `to` is specified outside `with` for **default mapping**.
...
> puts CustomCeramic.new(name: "Vase", size: 12, country: "Spain", description: "A beautiful vase").to_xml
