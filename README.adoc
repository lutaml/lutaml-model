...
`min`:: The minimum number of elements that must be included. Minimum value is `0`.
`max`:: The maximum number of elements that can be included.Maximum value is `Float::INFINITY`.
