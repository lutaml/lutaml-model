...
You can inherit from `Lutaml::Model::Collection` to define custom behavior for collections. Use the `collection_class` option to specify the custom collection class for an attribute.

[source,ruby]
----
class CustomPotCollection < Lutaml::Model::Collection
  def find_by_material(material)
    find { |pot| pot.material == material }
  end
end

class Studio < Lutaml::Model::Serializable
  attribute :pots, Pot, collection: true, collection_class: CustomPotCollection
end

studio = Studio.new(pots: [Pot.new(material: "clay"), Pot.new(material: "ceramic")])
puts studio.pots.find_by_material("clay") # => Pot instance with material: "clay"
----

If no `collection_class` is provided, the default `Lutaml::Model::Collection` is used.

There are some link:lib/lutaml/model/collection.rb[methods] overriden in `Lutaml::Model::Collection` class to return `Lutaml::Model::Collection` instance instead of `Array`, If you want to override the default behavior, you can inherit from `Lutaml::Model::Collection` and override the methods you need.
