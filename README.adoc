...
This example demonstrates converting between date formats with full
bidirectional support across different serialization formats.

.ISO week date format with bidirectional calculation
[example]
====
This example demonstrates conversions between two kinds of ISO 8601
representations:

* Basic calendar date: `[year][month][day]` [ISO 8601-1:2019, 5.2.2.1 a) Basic format]
* Explicit calendar date: `[yearE(m)][monthE][dayE]` [ISO 8601-2:2019, 7.2.2]
* Extended week date: `[year][“-”][“W”][week][“-”][day]` [ISO 8601-1:2019, 5.2.4.1 b) Extended format]

The example utilizes multi-format transforms:

* XML uses the Basic calendar date format
* YAML uses the Explicit calendar date format
* JSON uses the Extended week date format
* all formats parse back into `Lutaml::Model::Type::Date` objects
* enables round-trip transforms by serializing and deserializing with no data loss

[source,ruby]
----
# Multi-format date type with calculated transformations
class ComplexDate < Lutaml::Model::Type::Date
  # Parse ISO 8601-1 5.2.2.1 a) Basic format: 20250203
  def self.from_xml(value)
    return nil if value.nil? || value.empty?

    # Match basic format YYYYMMDD (8 digits)
    return nil unless value.match(/^\d{8}$/)
    ::Date.new(value[0..3].to_i, value[4..5].to_i, value[6..7].to_i)
  end

  # Serialize to ISO 8601-1 5.2.2.1 a) Basic format: 20250203
  def to_xml
    return nil unless value
    value.strftime('%Y%m%d')
  end

  # Parse ISO 8601-2 7.2.2 Explicit calendar date: 2025Y2M3D
  def self.from_yaml(value)
    return nil if value.nil? || value.empty?

    match = value.match(/^(\d{4})Y(\d{1,2})M(\d{1,2})D$/)
    return nil unless match
    ::Date.new(*match.slice[1,3].map(&:to_i))
  end

  # Serialize to ISO 8601-2 7.2.2 Explicit calendar date: 2025Y2M3D
  def to_yaml
    return nil unless value
    value.strftime('%Y-%m-%d')
  end

  # Parse ISO 8601-1 5.2.4.1 b) Extended week date: 2025-W05-3
  def self.from_json(value)
    return nil if value.nil? || value.empty?

    # Match pattern YYYY-W##-D
    match = value.match(/^(\d{4})-W(\d{2})-(\d)$/)
    return nil unless match

    # Date.commercial gives date from ISO week date components
    ::Date.commercial(*match.slice[1,3].map(&:to_i))
  end

  # Serialize to ISO 8601-1 5.2.4.1 b) Extended week date: YYYY-W##-D
  def to_json
    return nil unless value
...
class Schedule < Lutaml::Model::Serializable
  attribute :week_date, ComplexDate
...
# XML: Serializes to ISO 8601-1 5.2.2.1 a) Basic format: YYYYMMDD
schedule.to_xml
# => <schedule>
#      <date>20241225</date>
...
# YAML: Serializes to ISO 8601-2 7.2.2 Explicit calendar date: YYYY-MM-DD
schedule.to_yaml
# => ---
# => date: 2024Y12M25D
# => activity: Team Meeting

# JSON: Serializes to ISO 8601-1 5.2.4.1 b) Extended week date: YYYY-W##-D
schedule.to_json
# => {"weekDate":"2024-W52-3","activity":"Team Meeting"}

# Parse the YYYY-W##-D components:
# 2024: Year 2024
# W52: Week 52 of the year (prefixed with 'W')
# 3: Day 3 of the week (Wednesday, where Monday=1)

# Round-trip transformations work across all formats
parsed_xml = Schedule.from_xml(schedule.to_xml)
parsed_yaml = Schedule.from_yaml(schedule.to_yaml)
parsed_json = Schedule.from_json(schedule.to_json)

# All parsed dates are equivalent
parsed_xml.week_date == parsed_yaml.week_date == parsed_json.week_date
# => true

# Cross-format parsing examples:

# XML basic format: 20241225
xml_basic = '<schedule><date>20241225</date><activity>Meeting</activity></schedule>'
Schedule.from_xml(xml_basic).week_date
# => #<Date: 2024-12-25>

# YAML explicit format: 2024Y12M25D
yaml_explicit = "---\ndate: 2024Y12M25D\nactivity: Meeting"
Schedule.from_yaml(yaml_explicit).week_date
# => #<Date: 2024-12-25>

# JSON week format: 2024-W52-3
json_week = '{"weekDate":"2024-W52-3","activity":"Meeting"}'
Schedule.from_json(json_week).week_date
# => #<Date: 2024-12-25>

# Converting between formats maintains the same date but uses different representations
xml_schedule = Schedule.from_xml(xml_basic)
xml_schedule.to_yaml   # => "---\ndate: 2024-12-25\nactivity: Meeting"
xml_schedule.to_json   # => {"weekDate":"2024-W52-3","activity":"Meeting"}
----
====
...
The Reference type provides a mechanism for creating relationships between models. Instead of embedding full objects, references store lightweight keys that automatically resolve to actual objects when accessed.

==== Basic usage

Define reference attributes using the `ref:` syntax:
...
[source,ruby]
----
# Objects are automatically registered in the store when created
author = Author.new(id: "author-1", name: "John Doe")

# Create a book with a reference
book = Book.new(id: "book-1", title: "Great Book")
book.author_ref = "author-1"  # Assign the key directly

# Access the referenced object directly
puts book.author_ref.name  # => "John Doe" (automatically resolved)

# Collection of references
book.co_authors = ["author-1", "author-2"]
book.co_authors.each do |author|
  puts author.name if author  # Each element is the actual Author object
end

# Unresolved references return nil
book.author_ref = "non-existent-id"
puts book.author_ref.inspect  # => nil (not the key string)
----

==== Reference key accessor methods

Lutaml::Model automatically generates additional methods to access reference keys directly without resolving the objects:

[source,ruby]
----
# For single references: <attribute_name>_<key_attribute>
book.author_ref = "author-1"
book.author_ref_id     # => "author-1" (just the key)
book.author_ref.name   # => "John Doe" (resolved object)

# For collection references: <attribute_name>_<pluralized_key_attribute>
book.co_authors = ["author-1", "author-2"]
book.co_authors_ids         # => ["author-1", "author-2"] (just the keys)
book.co_authors.map(&:name) # => ["John Doe", "Jane Smith"] (resolved objects)

# Works with custom key attributes too
# attribute :category_ref, { ref: ["Category", :slug] }
# product.category_ref_slug  # => "electronics" (key accessor)

# Pluralization examples:
# :id → :ids, :name → :names, :category → :categories
----

==== Serialization

References serialize cleanly to just their key values:
...
When deserializing, the keys are automatically converted back to references that resolve transparently to the original objects when accessed.

For more details, see link:docs/reference_type.adoc[Reference Type Documentation].
