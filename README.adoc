...
The Reference type provides a mechanism for creating relationships between models through lazy-loaded references. Instead of embedding full objects, references store lightweight keys that resolve to actual objects when needed.
...
When deserializing, the keys are automatically converted back to Reference instances that can resolve to the original objects.
