...
class Product < Lutaml::Model::Serializable
  ...
  liquid do
    ...
    map "short_description", to: :truncated_description
  end
  ...
  def truncated_description(length = 50)
...
Your custom methods can accept parameters, with support for default values:
...
class Article < Lutaml::Model::Serializable
  ...
  liquid do
    map "custom_title", to: :format_title
    map "excerpt", to: :create_excerpt
    map "byline", to: :author_byline
  end

  def format_title(prefix = "Article")
    "#{prefix}: #{title}"
  end

  def create_excerpt(length = 100, suffix = "...")
    content.length > length ? "#{content[0, length]}#{suffix}" : content
  end

  def author_byline(format = "default")
    case format
    when "formal"
      "Written by #{author}"
    when "casual" 
...
drop = article.to_liquid

# Methods work with and without parameters
...
# Can also call with parameters in Ruby (not in templates)
puts drop.custom_title("Guide")  # => "Guide: Ruby Best Practices"
puts drop.create_excerpt(50, "...more")  # => "Lorem ipsum dolor sit amet...more"
----
...
class Schema < Lutaml::Model::Serializable
  ...
  def build_template_path(prefix = "templates")
...
class CustomSchemaDrop < BaseDropClass
  ...
  # Methods with parameters
  def transform_content(prefix = "CUSTOM", suffix = "END")
    "#{prefix}-#{@object.source}-#{suffix}"
  end
end
----
...
puts drop.transform_content  # => "CUSTOM-database settings-END"
