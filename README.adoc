...
It groups a set of element declarations, allowing them to be reused together. It permits only one child: `choice` or `sequence`.

Group supports (choice | sequence)

If the attributes defined directly in group or
the nested group | multiple choices | sequence are defined, `InvalidGroupError` will be
