...
Lutaml::Model XML adapters handle HTML entities differently:

**NokogiriAdapter**
+
Handles standard HTML named entities out of the box. You can use XML input or output with entities like `&copy;`, `&mdash;`, or `&nbsp;` and they will be correctly parsed and serialized.

**OxAdapter**
+
Also supports most standard named HTML entities. Entity references in XML will generally be correctly parsed and handled during serialization.

**OgaAdapter**
+
Currently does not expand or support named HTML entities other than the five XML predefined ones. Documents containing `&copy;`, `&reg;`, etc. will not decode those entities.

For best compatibility with entity-rich XML documents, use the NokogiriAdapter or OxAdapter.  
