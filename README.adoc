...
==== Understanding Types vs Serializable objects

Lutaml::Model supports two main approaches for defining custom data structures:

1. **Custom Types** (inherit from `Lutaml::Model::Type::Value`) - for primitive-like values
2. **Serializable objects** (inherit from `Lutaml::Model::Serializable`) - for complex objects with multiple attributes

The key differences are:

[cols="1,2,2"]
|===
| Aspect | Custom Types (`Type::Value`) | Serializable Objects (`Serializable`)
...
===== Quick Reference: Type vs Serializable
...
# ✅ SERIALIZABLE OBJECT - for complex objects with multiple attributes  
...
==== When to use Custom Types vs Serializable objects
...
==== Practical examples: Type vs Serializable
...
==== Summary: Custom Types best practices

When implementing custom types in Lutaml::Model:

1. **Inherit appropriately**: Use `Lutaml::Model::Type::Value` for completely new types, or extend built-in types like `Type::String`, `Type::Integer` for specialized behavior.

2. **Implement required methods**: Always implement `self.cast(value)` and `self.serialize(value)` at minimum.

3. **Add validation**: Use the `cast` method to validate and normalize input values.

4. **Handle format-specific serialization**: Override `to_xml`, `to_json`, etc. methods when different formats need different representations.

5. **Register reusable types**: Use `Lutaml::Model::Type.register(:symbol, YourType)` for types you'll use across multiple models.

6. **Choose the right approach**: 
   - Custom Types for primitive-like values with special behavior
   - Serializable objects for complex structures with multiple attributes

7. **Leverage inheritance**: Create type hierarchies for related but specialized types (like different currency types).

8. **Test thoroughly**: Custom types affect both parsing and serialization, so test with all formats you plan to use.

Custom types provide powerful capabilities for creating domain-specific value objects that integrate seamlessly with Lutaml::Model's serialization system while maintaining type safety and validation.
