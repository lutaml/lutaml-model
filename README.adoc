...
Lutaml::Model supports collection from xml attributes. A `delimiter` option for XML `map_attribute` is given, allowing you to control how values are split and joined when serializing/deserializing collections from a single XML attribute. This is useful when multiple values are stored in a single attribute, separated by a delimiter (e.g., `;`).
