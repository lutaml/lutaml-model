...
=== Attributes within Group

It groups a set of elements, allowing them to be collectively incorporated into complex type definitions as a single unit.

Group supports (choice | sequence)

If the attributes defined directly in group or
the nested group is defined or the
multiple choices are defined in group, `InvalidGroupError` will be
raised according to error.
+
[example]
====
group do
...
.Using the `group` option to define a collection of attributes
[example]
====
[source,ruby]
----
...
=== Attributes within Choice

The Choice option ensures that exactly one element from the specified group is included.

Choice supports (element | group | choice)*

.Using the `choice` option to define a set of attributes where exactly one can be used.
[example]
====
[source,ruby]
----
...
=== Attributes within Sequence

The sequence option ensures that the given elements must be in specified order.

Sequence supports (element | group | choice | sequence)*

.Using the `sequence` element to define a set of elements in desired order.
[example]
====
[source,ruby]
----
...
* `group` :: Validates the attributes incorporated as a group
* `choice` :: Validates that only one attribute specified
* `sequence` :: Validates that elements appear in specified order
...
klin = Klin.new(name: "Klin", degree_settings: [100, 200, 300], description: "one", prefix: "Ben", id: 1, age: 24)
klin.validate
# => []

klin = Klin.new(name: "Klin", degree_settings: [], description: "four", prefix: "Ben", nick_name: "Smith", age: 24, id: 1)
klin.validate
# => [
...
#      #<Lutaml::Model::InvalidChoiceError: Exactly one attribute must be specified in a choice>
#      #<Lutaml::Model::InvalidSequenceError: Attributes must be specified in defined order in the sequence>
#    ]
...
#     #<Lutaml::Model::ValueError: description must be one of [one, two, three]>,
#     #<Lutaml::Model::InvalidChoiceError: Exactly one attribute must be specified in a choice>,
#     #<Lutaml::Model::InvalidSequenceError: Elements must be present in the specified order>
#   ]
----
====
