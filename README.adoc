...
The `map` with `root_mappings` option is used for key-value data that is keyed using an
...
The `map` with `root_mappings` feature in practice parses the first data structure in the same
...
class SomeCollection < Lutaml::Model::Serializable
  ...
  json | yaml | toml | key_value do
    map to: :holder_of_collection_data, <1>
      root_mappings: { <2>
...
.Using the `map` with `root_mappings` option to map collection keys into individual models
[example]
====
Given this data:
...
.Using the `map` with `root_mappings` option to map collection keys and values into individual models
[example]
====
Given this data:
...
.Using the `map` with `root_mappings` option to map collection keys with complex data into individual models
[example]
====
