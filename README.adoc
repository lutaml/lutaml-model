...
* Union types for flexible attribute handling (see <<union-types>>)
...
Union types in LutaML::Model allow attributes to accept values of different types, providing flexibility in data modeling while maintaining type safety. This is particularly useful when working with APIs or data sources that may return different data structures for the same field depending on context.

=== Declaring Union Types

Union types are declared using the `union` option in attribute definitions:

[source,ruby]
----
class MyModel < Lutaml::Model::Serializable
  # Union of different model classes and primitive types
  attribute :content, { union: [TextContent, ImageContent, :string] }
end
----

=== Supported Union Members

Union types can include:

* **Serializable Model Classes**: Custom classes inheriting from `Lutaml::Model::Serializable`
* **Primitive Types**: `:string`, `:integer`, `:float`, `:boolean`, `:decimal`, etc.
* **Built-in LutaML Types**: Any type from the `Lutaml::Model::Type` namespace

[source,ruby]
----
class Product < Lutaml::Model::Serializable
  # Complex union with multiple types
  attribute :price, { union: [Money, :decimal, :string] }
  attribute :description, { union: [RichText, :string] }
  attribute :metadata, { union: [ProductMetadata, Hash, :string] }
end
----

=== Type Resolution

During deserialization, LutaML::Model attempts to cast the input value to each union type in order until one succeeds:
...
class Ceramic < Lutaml::Model::Serializable
  # Union tries TemperatureWithUnit first, then Temperature, then string
  attribute :firing_temperature, { union: [TemperatureWithUnit, Temperature, :string] }
end
----

The order of types in the union declaration is significant. LutaML::Model tries each type in sequence and uses the first successful match:

[source,ruby]
----
# This will try string first (might capture everything as string)
attribute :value, { union: [:string, ComplexType] }  # ⚠️ Problematic

# This will try ComplexType first (better)
attribute :value, { union: [ComplexType, :string] }  # ✅ Recommended
----

LutaML::Model automatically tracks which union type was successfully used during deserialization. This information is stored in the model instance and used during serialization to maintain type consistency.

[source,ruby]
----
yaml_data = "firing_temperature: Very Hot"
ceramic = Ceramic.from_yaml(yaml_data)

# Serialization uses the tracked type
ceramic.to_yaml  # => "firing_temperature: Very Hot" (preserves string format)
----

=== Format Support

Union types work seamlessly across all supported serialization formats:

[source,ruby]
----
class DataContainer < Lutaml::Model::Serializable
  attribute :content, { union: [StructuredData, :integer, :string] }
...
container = DataContainer.from_yaml(yaml_data)
container = DataContainer.from_json(json_data)
container = DataContainer.from_xml(xml_data)
container = DataContainer.from_toml(toml_data)
----
...
1. **Most Specific First**: Place complex types before primitive types
2. **Common Cases First**: Put frequently used types earlier in the union
3. **String Last**: Generally put `:string` last to avoid capturing everything

[source,ruby]
----
# Good: Specific types first
attribute :value, { union: [CustomModel, Hash, :integer, :string] }

# Avoid: String first (will capture almost everything)
attribute :value, { union: [:string, CustomModel, Hash] }
----

=== Collections with Union Types

Union types work with collections:

[source,ruby]
----
class MixedCollection < Lutaml::Model::Serializable
  attribute :items, { union: [Product, :string] }, collection: true
...
class APIResponse < Lutaml::Model::Serializable
  attribute :status, :string
  attribute :data, { union: [SuccessData, ErrorData, :string] }
  attribute :metadata, { union: [ResponseMetadata, :string] }
...
class Configuration < Lutaml::Model::Serializable
  attribute :database_config, { union: [DatabaseConfig, :string] }
end
----
...
The `XmlNamespace` class provides a
declarative way to define XML namespace metadata following W3C XML Namespace
...
=== Enhanced element declaration

==== `element` method

The `element` method is the primary way to
declare the XML element name for a model.
...
This is the clean, modern API for declaring element names.

.Using `element` to declare element name
[example]
====
[source,ruby]
----
...
==== `root` method as alias

The `root` method is maintained as a
backward-compatible alias to `element` that also supports the `mixed:` and
`ordered:` options.
...
`mixed`::: `true` to enable mixed content (text + elements), `false` otherwise (default)
`ordered`::: `true` to preserve element order, `false` otherwise (default)
...
==== `mixed_content` method

The `mixed_content` method explicitly enables
mixed content mode.
...
The `type_name` method sets an explicit type
name for XSD generation.
...
By default, type names are inferred as `ClassNameType`. Use this to override.
...
=== Migration from `no_root`

The `no_root` method is deprecated. For type-only models (models used only as
embedded types without their own element), simply omit the element declaration.
...
[`NoRootMappingError`](lib/lutaml/model/error/no_root_mapping_error.rb).
...
namespace will be applied to any element or attribute using this type, unless
overridden by explicit mapping namespace. Works for both serialization and
deserialization.

`xsd_type`:: Class-level directive that sets the XSD type name. If not specified,
defaults to `default_xsd_type` from the parent class (e.g., `xs:string` for
`Type::String`).

==== Deprecated syntax

IMPORTANT: The `namespace` directive for `Type::Value` classes is deprecated. Use `xml_namespace` instead.
...
Models can declare their default namespace using the `namespace` directive:
...
----
<1> Model-level namespace applies to the model's root element and child elements
(unless overridden)

.Using model-level namespace
[example]
====
[source,ruby]
----
...
class DctermsCreatedType < Lutaml::Model::Serializable
  ...
  xml do
    root 'created'
    map_attribute 'type', to: :type  # xsi: namespace from XsiTypeType
...
=== Type-level namespace declaration

Type::Value classes declare their namespace using the `xml_namespace` directive:

[source,ruby]
----
class EmailType < Lutaml::Model::Type::String
  xml_namespace EmailNamespace  # Declare namespace
  xsd_type 'EmailAddress'       # Set XSD type name
end
----
...
* Consistency with XmlNamespace pattern
...
----
<1> List of XmlNamespace classes to declare at root element
...
.Namespace consolidation with vCard
[example]
====
[source,ruby]
----
...
Without `namespace_scope`, each namespace would be declared locally where used:
...
The `Type::Reference` type can utilize
`:xsd_type` for proper XSD generation:
...
[[namespace-qualification-attribute-level]]
== Per-element and per-attribute namespace control

=== General

Individual elements and attributes can specify their own namespace via the
`namespace:` and `prefix:` options on mapping rules.

This allows mixing elements from different namespaces within a single model.

=== Element-level namespace
...
# Generate XSD
xsd = Lut<br/>aml::Model::Schema.to_xml(
  Contact,
