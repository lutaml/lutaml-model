...
The `key_value` method is a streamlined way to map all attributes for serialization into key-value formats such as JSON, YAML, and TOML. It simplifies defining mappings and improves code readability while maintaining flexibility.

[example]
Sets mappings for all key value formats JSON, YAML, and TOML

Syntax:

[source,ruby]
----
class ExampleModel < Lutaml::Model::Serializable
