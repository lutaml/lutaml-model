...
TODO: This case needs to be fixed for XML element name.
...
.Simple root collection in XML with models each containing an attribute `name`
[source,xml]
----
<name>Item One</name>
<name>Item Two</name>
...
.Simple root collection in YAML with models each containing an attribute `name`
[source,yaml]
----
---
- name: Item One
...
.Named collection in XML with models each containing an attribute `name`
[source,xml]
----
<names>
  <name>Item One</name>
...
.Named collection in YAML with models each containing an attribute `name`
[source,yaml]
----
---
names:
...
A named collection can be implemented as a non-collection model.
A root collection, however, cannot be represented using a non-collection model.

.Handling a named collection as a non-collection model
[example]
====
[source,ruby]
----
...
class TitleCollection < Lutaml::Model::Serializable
  attribute :titles, Title, collection: true
...
A model attribute that is a collection can be contained within a
...
There are two kinds of keyed collection depending on the type of the keyed value:

"keyed model collection":: the value is a "model instance"
...
`{attribute-of-instance}`:: The attribute of the model instance to order by. Values supported are:
...
The serialization format short name is defined as follows:
...
different data formats. For more information, see the
link:docs/custom_adapters.adoc[Custom Adapters Guide].
...
For more information See link:docs/custom_registers.adoc[Custom Registers Guide]
