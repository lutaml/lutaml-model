...
==== Passing Custom Arguments (metadata) to from_* Methods

You can pass custom arguments to `from_*` methods (e.g., `from_xml`, `from_<key-value>`) using the `metadata` option. These arguments will be forwarded to your custom deserialization methods when they accept 3 or more arguments. This is useful when you need to pass context-specific information or configuration to your custom methods.

[source,ruby]
----
# Custom method signature options:
# 1. Two arguments: (model, value) - metadata is NOT passed
# 2. Three arguments: (model, value, metadata) - metadata IS passed
----
...
class Product < Lutaml::Model::Serializable
  ...
  # Custom method accepting metadata (custom arguments)
...
# With metadata (custom arguments)
metadata = { prefix: "[", suffix: "]" }
product = Product.from_json(json_data, metadata: metadata)
product.name  # => "[Widget]"
----
====
...
class Order < Lutaml::Model::Serializable
  ...
  def order_id_from_xml(model, value, metadata)
    namespace = metadata&.dig(:namespace) || "default"
    model.order_id = "#{namespace}:#{value.first.text}"
  end
end
----
...
# With metadata (custom arguments)
metadata = { namespace: "ORD" }
order = Order.from_xml(xml_data, metadata: metadata)
...
[NOTE]
====
* `metadata` is only passed to custom methods that accept 3 or more arguments
* If your custom method accepts 2 arguments `(model, value)`, `metadata` will NOT be passed
* If your custom method accepts 3 arguments `(model, value, metadata)`, `metadata` will be passed only if it's provided (it will be `nil` if not provided)
* `metadata` can be any object (Hash, Array, String, etc.) - it's passed through as-is
* The method's arity (number of arguments) determines whether `metadata` is passed
====
...
class Example < Lutaml::Model::Serializable
  ...
  # 2 arguments - metadata NOT passed
  def data_from_json(model, value)
    model.data = value
  end

  # 3 arguments - metadata IS passed (if provided)
  def data_from_json(model, value, metadata)
    source = metadata&.dig(:source) || "default"
...
# Method with 2 arguments - metadata not used
instance1 = Example.from_json(json_data, metadata: { source: "api" })
instance1.data  # => "test" (metadata ignored)

# Method with 3 arguments - metadata is used
instance2 = Example.from_json(json_data, metadata: { source: "api" })
instance2.data  # => "api: test"
----
====

[example]
====
Using metadata with different data types:
...
class Processor < Lutaml::Model::Serializable
  ...
  def result_from_json(model, value, metadata)
    # metadata can contain any type of data
    timestamp = metadata&.dig(:timestamp)
    options = metadata&.dig(:options) || {}
...
processor = Processor.from_json(json_data, metadata: metadata)
