...
Format preservation is automatic and requires no code changes. The format is captured during parsing and stored as metadata on the model instance, then applied during serialization through the three-phase namespace architecture (Collector → Planner → Builder).
