...
==== Passing Custom Argument (state) to from_* and to_* Methods

You can pass custom arguments to `from_*` methods (e.g., `from_xml`, `from_<key-value>`) and `to_*` methods (e.g., `to_xml`, `to_<key-value>`) using the `state` option. This argument will be forwarded to your custom serialization and deserialization methods based on their method arity (number of arguments).

For information about custom method signatures, see <<Key-value data model serialization with custom methods>> and <<XML serialization with custom methods>>.

[source,ruby]
----
# Method signature detection based on arity:
# 
# Key-value formats (JSON, YAML, TOML, Hash):
#   from_* methods: (model, value) or (model, value, state)
#   to_* methods: (model, hash) or (model, hash, state)
#   State passed if method accepts 3+ arguments
#
# XML format:
#   from_* methods: (model, value) or (model, value, state)
#   to_* methods: (model, parent, doc) or (model, parent, doc, state)
#   State passed if from_* method accepts 3+ arguments
#   State passed if to_* method accepts 4+ arguments
----
...
# With state (custom arguments)
state = { prefix: "[", suffix: "]" }
product = Product.from_json(json_data, state: state)
product.name  # => "[Widget]"
----
====
...
class Order < Lutaml::Model::Serializable
  ...
  xml do
    root "Order"
    map_element "OrderId", to: :order_id, with: { from: :order_id_from_xml }
    map_element "Total", to: :total
  end

  def order_id_from_xml(model, value, state)
    namespace = state&.dig(:namespace) || "default"
    model.order_id = "#{namespace}:#{value.text}"
  end
end
----
...
# With state (custom arguments)
state = { namespace: "ORD" }
order = Order.from_xml(xml_data, state: state)
...
[NOTE]
====
* For key-value formats (JSON, YAML, TOML, Hash):
  ** `from_*` methods: state passed if method accepts 3+ arguments `(model, value, state)`
  ** `to_*` methods: state passed if method accepts 3+ arguments `(model, hash, state)`
* For XML format:
  ** `from_*` methods: state passed if method accepts 3+ arguments `(model, value, state)`
  ** `to_*` methods: state passed if method accepts 4+ arguments `(model, parent, doc, state)`
* If your custom method accepts fewer arguments than required, `state` will NOT be passed
* If `state` is not provided when calling `from_*` or `to_*` methods, it will be `nil` in your custom method
* `state` can be any object (Hash, Array, String, etc.) - it's passed through as-is
* The method's arity (number of arguments) determines whether `state` is passed
====

[example]
====
Using state with `to_*` methods (serialization):
...
class Product < Lutaml::Model::Serializable
  ...
  def name_to_json(model, hash, state)
    prefix = state&.dig(:prefix) || ""
    suffix = state&.dig(:suffix) || ""
    hash["name"] = "#{prefix}#{model.name}#{suffix}"
  end

  def name_from_json(model, value, state)
    prefix = state&.dig(:prefix) || ""
    suffix = state&.dig(:suffix) || ""
    model.name = value.sub(/^#{Regexp.escape(prefix)}/, '').sub(/#{Regexp.escape(suffix)}$/, '')
  end
end
----
...
# Without state
product.to_json  # => {"name":"Widget","price":19.99}

# With state
state = { prefix: "[", suffix: "]" }
product.to_json(state: state)  # => {"name":"[Widget]","price":19.99}
----
====

[example]
====
Using state with XML `to_*` methods (note: requires 4+ arguments):
...
class Order < Lutaml::Model::Serializable
  ...
  xml do
    root "Order"
    map_element "OrderId", to: :order_id, with: { to: :order_id_to_xml, from: :order_id_from_xml }
    map_element "Total", to: :total
  end

  def order_id_to_xml(model, parent, doc, state)
    namespace = state&.dig(:namespace) || ""
    el = doc.create_element("OrderId")
    doc.add_text(el, "#{namespace}#{model.order_id}")
    doc.add_element(parent, el)
  end

  def order_id_from_xml(model, value, state)
    namespace = state&.dig(:namespace) || ""
    model.order_id = value.first.text.sub(/^#{Regexp.escape(namespace)}/, '')
  end
end
----
...
[source,ruby]
----
order = Order.new(order_id: "12345", total: 99.99)

# With state
state = { namespace: "ORD-" }
xml = order.to_xml(state: state)
# => <Order><OrderId>ORD-12345</OrderId><Total>99.99</Total></Order>
----
====

[example]
====
Method arity determines state passing:
...
class Example < Lutaml::Model::Serializable
  ...
  json do
    map "data", to: :data, with: { from: :data_from_json }
  end

  # Method with 2 arguments - state NOT passed
  def data_from_json(model, value)
    model.data = value
  end
end
----
...
# Method accepts 2 arguments - state ignored
instance = Example.from_json(json_data, state: { source: "api" })
instance.data  # => "test" (state not passed)
----

[source,ruby]
----
class ExampleWithState < Lutaml::Model::Serializable
  attribute :data, :string
  ...
  # Method with 3 arguments - state IS passed (if provided)
  def data_from_json(model, value, state)
    source = state&.dig(:source) || "default"
...
# Method accepts 3 arguments - state is used
instance = ExampleWithMetadata.from_json(json_data, state: { source: "api" })
instance.data  # => "api: test"

# State is nil if not provided
instance2 = ExampleWithMetadata.from_json(json_data)
instance2.data  # => "default: test"
----
====
...
[source,ruby]
----
class Processor < Lutaml::Model::Serializable
  attribute :result, :string

  json do
    map "result", to: :result, with: { from: :result_from_json }
  end

  def result_from_json(model, value, state)
    # state can contain any type of data
    timestamp = state&.dig(:timestamp)
    options = state&.dig(:options) || {}
    
    prefix = options[:prefix] || ""
    model.result = "#{prefix}#{value}"
  end
end
----
...
[source,ruby]
----
json_data = '{"result": "processed"}'

state = {
  timestamp: Time.now,
  options: { prefix: "[", validate: true }
}
processor = Processor.from_json(json_data, state: state)
