...
==== Collection mapping with dynamic keys

NOTE: This feature is for key-value data model serialization and deserialization
only.

The `map_root` option is used for key-value data that is keyed using an
attribute value. In other words, the key of a key-value pair is actually the
value of an attribute that belongs to the value.

Simply put, the following two data structures are meant to have the same data:

.A YAML collection implemented as an object, each keyed by the `id` attribute
[source,yaml]
----
---
vase1:
...
.A YAML collection implemented as an array, with the `id` attribute in each
[source,yaml]
----
---
- id: vase1
...
NOTE: The first data structure ensures uniqueness of the `id` attribute.

The `map_root` feature in practice parses the first data structure in the same
way that you would access / manipulate the second data structure, while
retaining the serialization semantics of using an attribute as key.

Syntax:

[source,ruby]
----
class SomeCollection < Lutaml::Model::Serializable
  attribute :holder_of_collection_data, CollectionObjectType, collection: true

  json | yaml | toml | key_value do
    map_root to: :holder_of_collection_data, <1>
      root_mappings: { <2>
        key: :attribute_name_of_collection_object_type_for_key, <3>
        # `value` is Optional
        value: :attribute_name_of_collection_object_type_for_data <4>
      }
  end
end
----
<1> The `map_root` option indicates that this class represents the root of the
serialization object being passed in. (Mandatory)
...
.Using the `map_root` option to map collection keys into individual models
[example]
====
Given this data:
...
class CeramicCollection < Lutaml::Model::Serializable
  ...
  key_value do
    map_root to: :ceramics, # All data goes to the `ceramics` attribute
      root_mappings: {
...
> first_ceramic = ceramic_collection.first
...
# key the data. This is defined through the `map_root` method in
# CeramicCollection.
...
.Using the `map_root` option to map collection keys and values into individual models
[example]
====
Given this data:
...
.Using the `map_root` option to map collection keys with complex data into individual models
[example]
====
...
[source,ruby]
----
> ceramics_collection = CeramicsCollection.from_yaml(yaml)
=> #<CeramicsCollection:0x0000000107a2cf30
  @ceramics=
...
> first_ceramic = ceramics_collection.first
> first_ceramic.to_yaml
=>
# ---
# id: vase1
# type: vase
...
> new_collection = CeramicsCollection.new(ceramics: [
    Ceramic.new(ceramic_id: "vase1",
                ceramic_type: "vase",
                ceramic_details: CeramicDetails.new(
                  name: "Imperial Vase", insignia: "Tang Tianbao")
