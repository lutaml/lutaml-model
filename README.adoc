...
* `validates_all_have(field, message: nil)` - Ensures all items have the specified field with a non-blank value
* `validate_collection(&block)` - Define custom validation logic that receives the collection and errors object
