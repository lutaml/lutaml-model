...
The `map_root` option is used for key-value with dynamic root name (key).
...
----
json | yaml | toml do
  map_root to: :name_of_attribute, root_mappings: { root_key_of_hash: :key, name_of_attribute: :value_of_hash }
end
----

The `root_mappings` is specified keyword used for setting the key and value for hash.

.Using the `map_root` option to extract values from a key-value data model
[example]
====
